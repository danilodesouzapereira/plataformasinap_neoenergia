...
// ---------------------------------------------------------------------------
__fastcall TFormNIXELPA::TFormNIXELPA(TComponent *Owner, VTApl *apl_owner, TWinControl *parent)
	: TForm(Owner)
{
...
// ---------------------------------------------------------------------------
__fastcall TFormNIXELPA::~TFormNIXELPA(void)
{
	// destrói objetos
	if (nix)
	{
...
// ---------------------------------------------------------------------------
void __fastcall TFormNIXELPA::ActionGeraDadosExecute(TObject *Sender)
{
	// variaveis locais
...
// ---------------------------------------------------------------------------
void __fastcall TFormNIXELPA::ActionHelpExecute(TObject *Sender)
{
	// variaveis locais
	TForm *FormHelp;
...
// ---------------------------------------------------------------------------
void __fastcall TFormNIXELPA::ActionImportaDadosExecute(TObject *Sender)
{
...
// ---------------------------------------------------------------------------
void __fastcall TFormNIXELPA::ActionSalvaArquivoExecute(TObject *Sender)
{
	// varia´veis locais
	AnsiString filename;
	int count = 1;
...
// ---------------------------------------------------------------------------
void __fastcall TFormNIXELPA::cboxIAlClick(TObject *Sender)
{
	if (cboxIAl->Checked)
	{
...
// ---------------------------------------------------------------------------
void __fastcall TFormNIXELPA::cboxFDa4Click(TObject *Sender)
{
	if (cboxFDa4->Checked)
	{
...
void __fastcall TFormNIXELPA::cboxFDbtClick(TObject *Sender)
{
	if (cboxFDbt->Checked)
	{
...
// ---------------------------------------------------------------------------
void __fastcall TFormNIXELPA::clboxPatamarClick(TObject *Sender)
{
...
// ---------------------------------------------------------------------------
void __fastcall TFormNIXELPA::comboPatamarChange(TObject *Sender)
{ // variaveis locais
	VTPatamar *patamar;
	int indice;
...
// ---------------------------------------------------------------------------
bool __fastcall TFormNIXELPA::ContemMalha(void)
{
...
// ---------------------------------------------------------------------------
bool __fastcall TFormNIXELPA::ContemMalhaMT(VTMalhas *malhas)
{
	TList *lisLigacao;
...
// ---------------------------------------------------------------------------
void __fastcall TFormNIXELPA::ConfiguraInterfaceNix(void)
{
	if (nix != NULL)
	{
		...
		if (cboxCarreg->ItemIndex == 1)
		{
			nix->CarregamentoDirno = false;
		}
		else
		{
         	nix->CarregamentoDirno = true;
        }
		//verifica qual carregamento foi selecionado
		if (clboxPatamar->Checked)
...
// ---------------------------------------------------------------------------
void __fastcall TFormNIXELPA::DesabilitaCombo(void)
{
	comboPatamar->Clear();
...
// ---------------------------------------------------------------------------
bool __fastcall TFormNIXELPA::ExisteRedeBTEquivalente(void)
{ // variáveis locais
	VTRedes *redes = (VTRedes*)apl->GetObject(__classid(VTRedes));
	VTRede *rede;
	VTTipoRede *tipoRede;
...
// ---------------------------------------------------------------------------
bool __fastcall TFormNIXELPA::IniciaCBoxPatamar(void)
{ // variaveis locais
	VTPatamares *patamares = (VTPatamares*)apl->GetObject(__classid(VTPatamares));
	VTPatamar *patamar;
...
// ---------------------------------------------------------------------------
void __fastcall TFormNIXELPA::IniciaMapaParametro(void)
{
	mapa[1] = "6T";
	mapa[2] = "6K";
	mapa[3] = "10K";
...
// ---------------------------------------------------------------------------
/*
 void  __fastcall TFormNIXELPA::IniciaForm(void)
 {
 if (nix != NULL)
 {
 //configura se deve inserir valor de corrente nos elementos de protecao
 ...
 */
// ---------------------------------------------------------------------------
void __fastcall TFormNIXELPA::FormClose(TObject *Sender, TCloseAction &Action)
{
	// destrói o Form
	Action = caFree;
}
...
// ---------------------------------------------------------------------------
bool __fastcall TFormNIXELPA::SomenteMalhaDeTrafo(VTMalhas *malhas)
{
...
// ---------------------------------------------------------------------------
bool __fastcall TFormNIXELPA::ValidaFatorDemandaA4(void)
{ // variaveis locais
	bool sucesso = true;
	AnsiString string;
...
// ---------------------------------------------------------------------------
bool __fastcall TFormNIXELPA::ValidaFatorDemandaTrafo(void)
{ // variaveis locais
	bool sucesso = true;
	AnsiString string;
...
// ---------------------------------------------------------------------------
bool __fastcall TFormNIXELPA::ValidaInputForm(void)
{ // variaveis locais
	bool sucesso = true;
...
// ---------------------------------------------------------------------------
// eof
void __fastcall TFormNIXELPA::editFDbtMouseActivate(TObject *Sender, TMouseButton Button,
...
void __fastcall TFormNIXELPA::editFDa4MouseActivate(TObject *Sender, TMouseButton Button,
	TShiftState Shift, int X, int Y, int HitTest, TMouseActivate &MouseActivate)
