...
bool __fastcall TFormMain::AbreReclamacao(int tipo_estudo)
   ...
   //habilita alocaBC e alocaRT  //DVK 2016.03.22 somente para estudo MT
//   ActionAlocaBC->Enabled = true;
//   ActionAlocaRT->Enabled = true;
   ActionAlocaBC->Enabled = (tipo_estudo == estudoRECLAMACAO_MT);
   ActionAlocaRT->Enabled = (tipo_estudo == estudoRECLAMACAO_MT);
...
void __fastcall TFormMain::ActionCfgArranjoExecute(TObject *Sender)
   ...
   if (DLL_ConfiguraArranjos(this, apl, NULL))
...
void __fastcall TFormMain::ActionCfgCurvaTipicaExecute(TObject *Sender)
   ...
   if (DLL_ConfiguraCurvaDemanda(this, apl, NULL))
...
//---------------------------------------------------------------------------
void __fastcall TFormMain::ActionFlowCfgExecute(TObject *Sender)
   {
   //variáveis locais
   TForm     *Form;
   VTFlow    *flow    = (VTFlow*)apl->GetObject(__classid(VTFlow));
...
//---------------------------------------------------------------------------
void __fastcall TFormMain::ActionHelpExecute(TObject *Sender)
   {
   //variáveis locais
   TForm *form;
...
//---------------------------------------------------------------------------
void __fastcall TFormMain::AtualizaCaptions(void)
   {
   //variáveis locais
   VTProduto *produto = (VTProduto*)apl->GetObject(__classid(VTProduto));
   ...
   ActionSobreSINAPrnt->Caption = "Sobre o programa "  + produto->Codigo + "...";
   }
...
bool __fastcall TFormMain::ConfiguraFlow(void)
   ...
   flow->CronometroEnabled = MenuFlowCronometro->Checked;
   //habilita/desabilita otimizações (uso de Tramos)
   flow->TramoEnabled      = MenuFlowOtimizacao->Checked;
...
*/
//---------------------------------------------------------------------------
bool __fastcall TFormMain::CriaNovaBaseDado(AnsiString arq_dest)
   {
   //variáveis locais
   VTDir  *dir    = (VTDir*)apl->GetObject(__classid(VTDir));
   VTPath *path   = (VTPath*)apl->GetObject(__classid(VTPath));
...
void __fastcall TFormMain::ExibeNomeRede(void)
   ...
   StatusBar->SimpleText = ExtractFileName(path->ArqRede);
   }
