...
#include <Fontes\Importa\Valfase\VTValfase.h>
...
bool __fastcall TConversor::ExtractTXT(AnsiString filefullpath, int ind_arq, int num_arq)
   ...
   try{//salva apenas nome do arquivo
      ...
      if ((importa = NewObjImportaANEEL(apl)) == NULL)
         {
         LogErro(filename, "Erro ao criar Classe: VTImportaANEEL");
         return false;
         }
...
bool __fastcall TConversor::IniciaObjApl(void)
	...
	try{//cria objeto Apl
      ...
      apl->Add(DLL_NewObjRadial(apl));
		//cria objeto PreMonta
		apl->Add(pre_monta = NewObjPreMonta(apl));
		//define opções de PreMonta
		pre_monta->DefineDataReferencia(opcao->date);
		pre_monta->DefineSubestacaoComPrimarios(opcao->sub.topologia == opSUB_TROCAR);
		pre_monta->DefineFormatoCampoAcoplamento(opcao->sec.topologia);
		}catch(Exception &e)
			{
			return(false);
         }
...
void __fastcall TConversor::LogInicia(void)
   ...
   //inicia Log resumo
   log_resumo->Open(path->DirTmp() + "\\ImportaANEEL_Resumo.log");
   //inicia Log completo
   if (log_completo)
      {//abre arquivo
      log_completo->Open(path->DirTmp() + "\\ImportaANEEL_Detalhado.log");
...
void __fastcall TConversor::VerificaFases(void)
   ...
   try{//cria objeto VTValfase
      valfase = NewObjValfase(apl);
      valfase->CorrigeFases = opcao->corrige_fase;
      valfase->Executa();
      if ((num_eqpto = valfase->ExisteEqptoComFaseInvalida()) > 0)
         ...
         //preenche log
         lisEXT = valfase->LisEqptoComFaseInvalida();
   ...
   if(valfase) {delete valfase; valfase = NULL;}
   }
...
void __fastcall TConversor::VerificaPerdaFerro(void)
   ...
   try{//cria objeto VTValPerdafe
      valperdafe = NewObjValPerdafe(apl);
      valperdafe->Executa();
   ...
   //destrói objeto criado
   if (valperdafe) {delete valperdafe; valperdafe = NULL;}
   }
