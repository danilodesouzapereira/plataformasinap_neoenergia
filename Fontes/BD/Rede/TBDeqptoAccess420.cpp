...
{
	return (new TBDeqptoAccess420(apl_owner, bd_conn));
}

// ---------------------------------------------------------------------------
__fastcall TBDeqptoAccess420::TBDeqptoAccess420(VTApl *apl_owner, VTSNPDBConn *bd_conn)
...
// ---------------------------------------------------------------------------
__fastcall TBDeqptoAccess420::~TBDeqptoAccess420(void)
{
	// nada a fazer
}

// ---------------------------------------------------------------------------
bool __fastcall TBDeqptoAccess420::SPSNP_rdBateria(VTEstudo* estudo, bool bolUseRedeTmp,
	bool bolObra)
{
	// a tabela nao existe
	return true;
...
// ---------------------------------------------------------------------------
bool __fastcall TBDeqptoAccess420::SPSNP_rdEstudo(void)
{ //igual versao 410
	bool sucesso = true;
	AnsiString strlog;
	AnsiString strSQL;
...
// ---------------------------------------------------------------------------
bool __fastcall TBDeqptoAccess420::SPSNP_rdEstudoAlternativa(VTEstudo* estudo)
{//igual versao 40
	bool sucesso = true;
	double tipo_padrao;
	AnsiString strlog;
...
// ---------------------------------------------------------------------------
bool __fastcall TBDeqptoAccess420::SPSNP_rdEstudoObraAtribExt(VTEstudo* estudo)
{ // DVK 2018.02.20 leitura do campo Descricao
	bool sucesso = true;
	double tipo_padrao;
	AnsiString strlog;
	AnsiString strSQL;
...
// ---------------------------------------------------------------------------
bool __fastcall TBDeqptoAccess420::SPSNP_rdMunicipio(void)
{
	// a tabela nao existe
	return true;
}
...
// ---------------------------------------------------------------------------
bool __fastcall TBDeqptoAccess420::SPSNP_rdRegulador(VTEstudo* estudo, bool bolUseRedeTmp,
	bool bolObra)
{//campo tensao inversa, não possui largura de banda nem relação de tap
	bool sucesso = true;
	AnsiString strlog;
	AnsiString strSQL;
	TADOQuery* ptrQryDS = NULL;
		...
		try
		{
			...
			for (int i = 0; i < ptrQryDS->RecordCount; i++)
			{
				...
				if (ndxBarra == 0)
				{
					...
					// Como não eh a versao nova  - adot zero na largura de banda
