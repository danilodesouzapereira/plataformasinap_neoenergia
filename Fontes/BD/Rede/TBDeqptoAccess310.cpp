...
{
	return (new TBDeqptoAccess310(apl_owner, bd_conn));
}

// ---------------------------------------------------------------------------
__fastcall TBDeqptoAccess310::TBDeqptoAccess310(VTApl *apl_owner, VTSNPDBConn *bd_conn)
...
// ---------------------------------------------------------------------------
__fastcall TBDeqptoAccess310::~TBDeqptoAccess310(void)
{
	// nada a fazer
}

// ---------------------------------------------------------------------------
bool __fastcall TBDeqptoAccess310::SPSNP_rdBateria(VTEstudo* estudo, bool bolUseRedeTmp,
	bool bolObra)
{
	// a tabela nao existe
	return true;
...
// ---------------------------------------------------------------------------
bool __fastcall TBDeqptoAccess310::SPSNP_rdEstudo(void)
...
// ---------------------------------------------------------------------------
bool __fastcall TBDeqptoAccess310::SPSNP_rdEstudoAlternativa(VTEstudo* estudo)
{//nao tem taxa de falha
	bool sucesso = true;
	double tipo_padrao;
	AnsiString strlog;
	AnsiString strSQL;
			...
			for (int i = 0; i < ptrQryDS->RecordCount; i++)
			{
				...
				if (intObraId > 0)
				{
					if (intObraId != intObraIdAnterior)
					{
						...
						str_obra.altera_taxa_falha = false;
							//bool(ptrQryDS->FieldByName("bolAlteraTaxaFalha")->AsInteger);
						str_obra.taxa_falha_ins = 0.;//ptrQryDS->FieldByName("fltTaxaFalhaIns")->AsFloat;
						str_obra.taxa_falha_sub = 0.;//ptrQryDS->FieldByName("fltTaxaFalhaSub")->AsFloat;
...
// ---------------------------------------------------------------------------
bool __fastcall TBDeqptoAccess310::SPSNP_rdEstudoObraAtribExt(VTEstudo* estudo)
{//não possuia integração com ARIES/VIP
	bool sucesso = true;
	return sucesso;
}

// ---------------------------------------------------------------------------
bool __fastcall TBDeqptoAccess310::SPSNP_rdMunicipio(void)
{
	// a tabela nao existe
	return true;
}
...
// ---------------------------------------------------------------------------
bool __fastcall TBDeqptoAccess310::SPSNP_rdRegulador(VTEstudo* estudo, bool bolUseRedeTmp,
	bool bolObra)
