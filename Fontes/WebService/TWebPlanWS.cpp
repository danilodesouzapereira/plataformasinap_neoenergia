...
//---------------------------------------------------------------------------
//função para criar objeto VTWebPlanWS
VTWebPlanWS *NewObjWebPlanWS(VTApl *apl)
{
    return(new TWebPlanWS(apl));
}

//---------------------------------------------------------------------------
__fastcall TWebPlanWS::TWebPlanWS(VTApl *apl)
{
    //salva ponteiro
    this->apl = apl;
    //cria objetos
...
//---------------------------------------------------------------------------
__fastcall TWebPlanWS::~TWebPlanWS(void)
{
    //destrói objetos
    if (lines)
    {
...
bool __fastcall TWebPlanWS::ImportaPadrao(AnsiString arq_txt)
{
    //variáveis locais
...
//      port    = lines->Strings[3] + "Port";
        ...
        ListarPadraoConstrutivoWebService = 
            GetListarPadraoConstrutivoWebService(wsdl_address.c_str(), service.c_str(), port.c_str());
...
bool __fastcall TWebPlanWS::ImportaGrupo(AnsiString arq_txt)
{
    //variáveis locais
    AnsiString      wsdl_address, service, port, ext_wsdl;
    AnsiString      txt;
    ...
    int             dominio_id;
    grupo          *pgrupo;
    listarResponse  vet_grupo;
    _di_ListarGrupoWebService ListarGrupoWebService;

    try
    {
        //cria objeto ListarBrupoWebService para acessar o WebService
        //ListarGrupoWebService = GetListarGrupoWebService();
        //obtém domínio da Empresa
        if (!LeDominio(dominio_id))
            dominio_id = 1;
        //obtém endereço do WebService cadastrado no registro do Windows
        if (!LeEnderecoWSGrupo(wsdl_address))
            return(false);
        //inclui o string "?wsdl" no endereço, se ele ainda não tiver
        ext_wsdl = "?wsdl";
        if (wsdl_address.AnsiPos(ext_wsdl) <= 0)
...
//      port    = lines->Strings[3] + "Port";
        ...
        ListarGrupoWebService = 
            GetListarGrupoWebService(wsdl_address.c_str(), service.c_str(), port.c_str());
        //obtém dados do WebService e grava no arquivo txt
        vet_grupo = ListarGrupoWebService->listar(dominio_id);
        //lê grupos e salva em lines
        lines->Clear();
        for (int n = 0; n < vet_grupo.Length; n++)
        {
            pgrupo = vet_grupo[n];
            //monta uma linha com dados do grupo
            txt  = IntToStr(pgrupo->id) + separador;
            txt += pgrupo->nome         + separador;
            lines->Add(txt);
        }
        //salva grupos no arquivo
        lines->SaveToFile(arq_txt);
        //não destrói objeto porque ocorre Access Violation
        //delete ListarGrupoWebService;
        return(true);
    }
    catch(Exception &e)
    {
...
//---------------------------------------------------------------------------
bool __fastcall TWebPlanWS::LeDominio(int &dominio_id)
{
    //variáveis locais
    bool         sucesso = false;
    AnsiString   txt;
...
//---------------------------------------------------------------------------
bool __fastcall TWebPlanWS::LeEnderecoWSPadrao(AnsiString &wsdl)
{
    //variáveis locais
    bool         sucesso = false;
    VTRegistro  *registro;
...
//---------------------------------------------------------------------------
// https://sinapsisenergia.teamwork.com/#/tasks/17302005
bool __fastcall TWebPlanWS::LeEnderecoWSGrupo(AnsiString &wsdl)
{
    //variáveis locais
    bool         sucesso = false;
    VTRegistro  *registro;
...
//      if ((registro = NewObjRegistro("Sinap")) != NULL)
        if ((registro = DLL_NewObjRegistro("Sinapsis")) != NULL)
        {
            sucesso = registro->ExisteInfo("IPE", "ws_grupo", wsdl);
            //destrói objeto registro
            delete registro;
        }
    }
