...
{ // variaveis locais
	...
	tema_Sinal->ConverteLatLonGMStoSINAP(listSTR);
	// atualiza o grafico
	CalculaDensidadeMTSinal();
	tema_Sinal->FaixaValoresCelPlanner();
	tema_Sinal->MontaFaixaSinal();
...
void __fastcall TFormMT_Sinal::ImportaTxt(AnsiString filename)
{
	...
	{ // cria nova lista de importações
		...
		// converte as coordenadas em de graus minutos segundos para a coordenada usada no sinap
		tema_Sinal->ConverteLatLonGMStoSINAP(listSTR);
...
//---------------------------------------------------------------------------
void  __fastcall TFormMT_Sinal::PreencheTextoForm(void)
{
	AnsiString *texto;
	AnsiString strFCMSCPENGLISH[sfmscpCOUNT] =
	{
		"CelPlanner: Coverage Map",
		"Close",
		"Import",
		"Edit Range Limits",
		"Help",
	...
	AnsiString strFCMSCPPORTUGUES[sfmscpCOUNT] =
	{
		"CelPlanner: Mapa de Cobertura",
		"Fechar",
		"Importar",
		"Cfg. Limites",
		"Ajuda",
	...
	if(IS_ENGLISH == 1)
	{
		texto = &strFCMSCPENGLISH[0];
		//Acerta alguns tamanhos
		LView->Column[0]->Width = 50;
		LView->Column[1]->Width = 90;
		LView->Column[2]->Width = 90;
		LView->Column[3]->Width = 50;
	}
	else
	{
		texto = &strFCMSCPPORTUGUES[0];
		//Acerta alguns tamanhos
		LView->Column[0]->Width = 60;
		LView->Column[1]->Width = 100;
		LView->Column[2]->Width = 100;
		LView->Column[3]->Width = 50;
	}
	//titulo
	this->Caption = texto[sfmscpTITLE];
	butFecha->Caption = texto[sfmscpCLOSE];
	butImport->Caption = texto[sfmscpIMPORT];
	tbEditRange->Caption = texto[sfmscpEDTRANGE];
	butHelp->Caption = texto[sfmscpHELP];
	gboxImportSettings->Caption = texto[sfmscpIMPSETTINGS];
	labelLatOff->Caption = texto[sfmscpLATITUDE] + " " + texto[sfmscpOFFSETUNIT] + ":";
	labelLonOff->Caption = texto[sfmscpLONGITUDE] + " " + texto[sfmscpOFFSETUNIT] + ":";
	GBoxRedeVisible->Caption = texto[sfmscpVIEWOPTIONS];
	lblShow->Caption = texto[sfmscpSHOW] + ":";
	CheckBoxRedeVisible->Caption = texto[sfmscpSHOW] + " " + texto[sfmscpELETRICNET];
	lblGrid->Caption = texto[sfmscpGRIDSIZE]+ " " + texto[sfmscpGRIDSIZEUNIT] + ":";
	gbRanges->Caption = texto[sfmscpRANGES];
	cboxAutoRange->Caption = texto[sfmscpAUTORANGE];
	LabelSignalLevel->Caption = texto[sfmscpLEVEL];
	ButCorConsMaior->Caption = texto[sfmscpBETTER];
	ButCorConsMenor->Caption = texto[sfmscpWORST];
	LView->Column[0]->Caption = texto[sfmscpRANGE];
	LView->Column[1]->Caption = texto[sfmscpINFLIMIT]+ " " + texto[sfmscpSIGNALUNIT];
	LView->Column[2]->Caption = texto[sfmscpSUPLIMIT]+ " " + texto[sfmscpSIGNALUNIT];
	LView->Column[3]->Caption = texto[sfmscpCOLOR];
