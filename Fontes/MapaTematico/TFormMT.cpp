...
// ---------------------------------------------------------------------------
__fastcall TFormMT::TFormMT(TComponent *Owner, VTApl *apl_owner) : TForm(Owner)
{
	// variáveis locais
	...
	apl = apl_owner;
	// insere ActionList
	sinap_child->FormStkAdd("SINAP: Mapas Temáticos", ActionListMapaTematico, false);
}
...
//	// RBAC
//	if (!rbac->ExistePermissaoModulo(modMAPAAI))
//	{
...
void __fastcall TFormMT::ActionMP_CCSExecute(TObject *Sender)
{
	...
	// RBAC
	if (!rbac->ExistePermissaoModulo(modMAPACC))
	{
		Aviso("Sua licença de uso não tem permissão para " + rbac->GetDescricaoOperacao(opREDE));
		return;
	}
...
void __fastcall TFormMT::ActionMT_DensidadeCargaExecute(TObject *Sender)
{
	...
	// RBAC
	if (!rbac->ExistePermissaoModulo(modMAPADC))
	{
		Aviso("Sua licença de uso não tem permissão para " + rbac->GetDescricaoOperacao(opREDE));
		return;
	}
...
void __fastcall TFormMT::ActionMT_EnergiaExecute(TObject *Sender)
{
	...
	// RBAC
	if (!rbac->ExistePermissaoModulo(modMAPATENSAO))
	{
		Aviso("Sua licença de uso não tem permissão para " + rbac->GetDescricaoOperacao(opREDE));
		return;
	}
...
void __fastcall TFormMT::ActionMT_PerfilFatpotExecute(TObject *Sender)
{
	...
	// RBAC
	if (!rbac->ExistePermissaoModulo(modMAPAFP))
	{
		Aviso("Sua licença de uso não tem permissão para " + rbac->GetDescricaoOperacao(opREDE));
		return;
	}
...
void __fastcall TFormMT::ActionMT_PerfilTensaoExecute(TObject *Sender)
{
	...
	// RBAC
	if (!rbac->ExistePermissaoModulo(modMAPAPT))
	{
		Aviso("Sua licença de uso não tem permissão para " + rbac->GetDescricaoOperacao(opREDE));
		return;
	}
...
bool __fastcall TFormMT::RedeGeoreferenciada(void)
{
	...
	if (IsDoubleZero(area->Width()))
		return (false);
	if (IsDoubleZero(area->Height()))
		return (false);
	return (true);
}
