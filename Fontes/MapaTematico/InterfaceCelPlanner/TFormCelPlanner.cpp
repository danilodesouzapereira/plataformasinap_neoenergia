...
//---------------------------------------------------------------------------
void  __fastcall TFormCelPlanner::PreencheTextoForm(void)
{
	AnsiString *texto;
	AnsiString strFCECPENGLISH[sfcpCOUNT] =
	{
		"CelPlanner Exporter",
		"Create Data",
		"Save to File",
	...
	AnsiString strFCECPPORTUGUES[sfcpCOUNT] =
	{
		"Exportador CelPlanner",
		"Criar dados",
		"Exportar dados",
		"Configuração",
		"Alimentador:",
	...
	if(IS_ENGLISH == 1)
		{texto = &strFCECPENGLISH[0];}
	else
		{texto = &strFCECPPORTUGUES[0];}
	//titulo
	this->Caption = texto[sfcpTITLE];
	cboxFeeder->Text = texto[sfcpALL];
	btnCreateData->Caption = texto[sfcpCREATEDATA];
	btnSaveFile->Caption = texto[sfcpSAVETOFILE];
	gbConfig->Caption = texto[sfcpSETTINGS];
	lblFeeder->Caption = texto[sfcpSELFEEDER];
	radioCoordenadas->Caption = texto[sfcpCOORDSYS];
	gbCargas->Caption = texto[sfcpLOAD];
	gbChaves->Caption = texto[sfcpSWITCHES];
	gbSubestacao->Caption = texto[sfcpSUBS];
	gbGeracao->Caption = texto[sfcpGENS];
	gbCapacitores->Caption = texto[sfcpCAPS];
	gbTipoChave->Caption = texto[sfcpSWITCHTYPES];
	gbReguladores->Caption = texto[sfcpVOLTAGEREG];
	//checkbox
	checkCargas->Caption = texto[sfcpEXPORT] + texto[sfcpLOAD];
	checkChaves->Caption = texto[sfcpEXPORT] + texto[sfcpSWITCHES];
	checkSubestacao->Caption = texto[sfcpEXPORT] + texto[sfcpSUBS];
	checkGeradores->Caption = texto[sfcpEXPORT] + texto[sfcpGENS];
	checkCapacitores->Caption = texto[sfcpEXPORT] + texto[sfcpCAPS];
	checkReligadores->Caption = texto[sfcpADD] + texto[sfcpRECLOSERS];
	checkSeccionadoras->Caption = texto[sfcpADD] + texto[sfcpSWITCHGEARS];
	checkReguladores->Caption = texto[sfcpEXPORT] + texto[sfcpVOLTAGEREG];
}
//---------------------------------------------------------------------------
