...
// ---------------------------------------------------------------------------
void __fastcall TFormMT_PFP::ChartInsereFatorPotencia(VTLigacao *ligacao, VTBarra *barra,
	int valor_x, int ind_pat)
{
	// variáveis locais
	int ind_bar;
	double fatpot;
...
// ---------------------------------------------------------------------------
void __fastcall TFormMT_PFP::LViewBarraInsereFatorPotencia(VTLigacao *ligacao, VTBarra *barra,
	double dist_m, int ind_pat)
{
	// variáveis locais
	int ind_bar;
	int vet_fase[MAX_FASE] =
	...
	fatpot = 2;
	ind_fase_min = indFASE_A; // DVC (24/04/2018) Inicialização da variavel- indFASE_A=0
	for (int ind_fase = indFASE_A; ind_fase <= indFASE_C; ind_fase++)
	{ // verifica se a Barra possui a fase
		if (barra->resflow->Fases & vet_fase[ind_fase])
		{ // verifica se existe "potencia" circulando na fase
	...
	if (fpIndutivo)
	{
		Item->SubItems->Add("Indutivo");
	} // FKM - 2015.09.10
	else
	{
		Item->SubItems->Add("Capacitivo");
	} // FKM - 2015.09.10
}
...
// ---------------------------------------------------------------------------
void __fastcall TFormMT_PFP::LViewCompare(TObject *Sender, TListItem *Item1, TListItem *Item2,
...
VTRede* __fastcall TFormMT_PFP::RedeDaBarra(VTBarra *barra)
{
	...
	{ // verifica se a Rede contém a Barra indicada
		if (dist.rede->ExisteBarra(barra))
			return (dist.rede);
	}
	// reinicia lisEQP
	for (int n = 0; n < lisREDE->Count; n++)
