...
{ {1, 5, ""}, // número
    ...
    {74, 76, "0"}, // área
    {77, 80, "1.0"}, // tensão para definição de carga
    {81, 81, "0"}, // modo de visualização
    {82, 84, ""}, // agregador 1
    {85, 87, ""}, // agregador 2
    {88, 90, ""}, // agregador 3
    {91, 93, ""}, // agregador 4
    ...
    {103, 105, ""}, // agregador 8
    {106, 108, ""}, // agregador 9
    {109, 111, ""}}; // agregador 10

strCAMPO cartao_DLIN[] =
{ {1, 5, ""}, // da barra
    {6, 6, "L"}, // abertura da barra (não utilizado)
    {8, 8, "A"}, // operação
    {10, 10, "L"}, // abertura para barra (não utilizado)
    {11, 15, ""}, // para barra
    {16, 17, ""}, // circuito
    {18, 18, "L"}, // estado
    {19, 19, "F"}, // proprietário (da barra)
    ...
    {100, 102, ""}, // agregador 8
    {103, 105, ""}, // agregador 9
    {106, 108, ""}}; // agregador 10

strCAMPO cartao_DGBT[] =
{ {1, 2, ""}, // grupo
    {4, 8, "1."}}; // tensão

strCAMPO cartao_DINJ[] =
{ {1, 5, ""}, // número
    {7, 7, "A"}, // operação
...
strCAMPO cartao_DBSH[] =
{ {1, 5, ""}, // da barra
    {7, 7, "A"}, // operação
    {9, 13, ""}, // para barra
    {15, 16, "1"}, // circuito
    {18, 18, "C"}, // modo de controle
    {20, 23, ""}, // tensão mínima
    {25, 28, ""}, // tensão máxima
    {30, 34, ""}, // barra controlada
    {36, 41, "0.0"}, // injeção reativa inicial
    {43, 43, "C"}, // tipo do controle
    {45, 45, "N"}, // apaga dados dbar
    {47, 51, ""}}; // extremidade

strCAMPO cartao_DBSH_2[] =
{ {1, 2, ""}, // grupo ou banco
    {5, 5, "A"}, // operação
    {7, 7, "L"}, // estado
    {9, 11, "1"}, // unidades
    {13, 15, ""}, // unidades em operação
    {17, 22, ""}}; // valor reator ou cap.
...
bool __fastcall TAnarede::Executa(TStrings *Lines, int num_area)
{
    ...
    for (int n = 0; n < sizeof(ja_tratou) / sizeof(bool); n++)
        ja_tratou[n] = false;
    ...
    { // verifica se é linha de comentario
        ...
        if (FimDeBloco(Lines->Strings[n]))
            ...
            case estDBAR: // procura cartão que identifica fim dos dados de Barras
                ...
                if (!pre_monta->InsereDBAR(strList))
                    return (false);
                break;
            case estDLIN:
                LeCartaoDLIN(Lines->Strings[n]);
                if (!pre_monta->InsereDLIN(strList))
                    return (false);
                break;
            case estDGBT:
                LeCartaoDGBT(Lines->Strings[n]);
                if (!pre_monta->InsereDGBT(strList))
                    return (false);
                break;
            case estDINJ:
                LeCartaoDINJ(Lines->Strings[n]);
                if (!pre_monta->InsereDINJ(strList))
                    return (false);
                break;
            case estDGBA:
                LeCartaoDGBA(Lines->Strings[n]);
                if (!pre_monta->InsereDGBA(strList))
                    return (false);
                break;
            case estDBSH:
                LeCartaoDBSH(Lines->Strings[n]);
                n++;
                while (Lines->Strings[n].Pos("FBAN") <= 0)
                { // verifica se é linha de comentario
                    if (!Comentario(Lines->Strings[n]))
                    { // le continuação do DBSH
                        LeCartaoDBSH2(Lines->Strings[n]);
                    }
                    n++;
                }
                if (!pre_monta->InsereDBSH(strList))
                    return (false);
                break;
            default:
...
int __fastcall TAnarede::InicioDeBloco(AnsiString txt)
{
    ...
    // verifica DBAR
    if (txt.AnsiCompareIC("DBAR") == 0)
        return (JaTratou(estDBAR) ? estERRO : estDBAR);
    if (txt.AnsiCompareIC("DLIN") == 0)
        return (JaTratou(estDLIN) ? estERRO : estDLIN);
    if (txt.AnsiCompareIC("DGBT") == 0)
        return (JaTratou(estDGBT) ? estERRO : estDGBT);
    if (txt.AnsiCompareIC("DINJ") == 0)
        return (JaTratou(estDINJ) ? estERRO : estDINJ);
    if (txt.AnsiCompareIC("DGBA") == 0)
        return (JaTratou(estDGBA) ? estERRO : estDGBA);
    if (txt.AnsiCompareIC("DBSH") == 0)
        return (JaTratou(estDBSH) ? estERRO : estDBSH);
    if (txt.AnsiCompareIC("FIM") == 0)
        return (estFIM);
    return (estNONE);
}
...
{
    // variáveis locais
    char *cmd[sizeof(ja_tratou) / sizeof(bool)] =
    {"DBAR", "DLIN", "DGBT", "DINJ", "DGBA", "DBSH", "NONE", "ERRO", "FIM"};
...
void __fastcall TAnarede::LeCartaoDBAR(AnsiString txt)
{
    ...
    for (int n = 0; n < sizeof(cartao_DBAR) / sizeof(strCAMPO); n++)
    {
        ...
        if (campo.IsEmpty())
            campo = cartao_DBAR[n].val_default;
        strList->Add(campo);
    }
}

// ---------------------------------------------------------------------------
void __fastcall TAnarede::LeCartaoDBSH(AnsiString txt)
{
    // variáveis locais
    int ind, count;
    AnsiString campo;
    ...
    strList->Clear();
    // lê campos do cartão
    for (int n = 0; n < sizeof(cartao_DBSH) / sizeof(strCAMPO); n++)
    {
        ind = cartao_DBSH[n].col_ini;
        count = cartao_DBSH[n].col_fim - cartao_DBSH[n].col_ini + 1;
        campo = (txt.SubString(ind, count)).Trim();
        if (campo.IsEmpty())
            campo = cartao_DBSH[n].val_default;
        strList->Add(campo);
    }
}

// ---------------------------------------------------------------------------
void __fastcall TAnarede::LeCartaoDBSH2(AnsiString txt)
{
    // variáveis locais
    int ind, count;
    AnsiString campo;
    ...
    // limpa StringList
    // strList->Clear();
    // lê campos complementares do cartão
    for (int n = 0; n < sizeof(cartao_DBSH_2) / sizeof(strCAMPO); n++)
    {
        ind = cartao_DBSH_2[n].col_ini;
        count = cartao_DBSH_2[n].col_fim - cartao_DBSH_2[n].col_ini + 1;
        campo = (txt.SubString(ind, count)).Trim();
        if (campo.IsEmpty())
            campo = cartao_DBSH_2[n].val_default;
        strList->Add(campo);
    }
}
...
void __fastcall TAnarede::LeCartaoDGBA(AnsiString txt)
{
    ...
    for (int n = 0; n < sizeof(cartao_DGBA) / sizeof(strCAMPO); n++)
    {
        ...
        campo = (txt.SubString(ind, count)).Trim();
        if (campo.IsEmpty())
            campo = cartao_DGBA[n].val_default;
        strList->Add(campo);
    }
}
...
void __fastcall TAnarede::LeCartaoDGBT(AnsiString txt)
{
    ...
    for (int n = 0; n < sizeof(cartao_DGBT) / sizeof(strCAMPO); n++)
    {
        ...
        campo = (txt.SubString(ind, count)).Trim();
        if (campo.IsEmpty())
            campo = cartao_DGBT[n].val_default;
        strList->Add(campo);
    }
}
...
void __fastcall TAnarede::LeCartaoDINJ(AnsiString txt)
{
    ...
    for (int n = 0; n < sizeof(cartao_DINJ) / sizeof(strCAMPO); n++)
    {
        ...
        campo = (txt.SubString(ind, count)).Trim();
        if (campo.IsEmpty())
            campo = cartao_DINJ[n].val_default;
        strList->Add(campo);
    }
}
...
void __fastcall TAnarede::LeCartaoDLIN(AnsiString txt)
{
    ...
    for (int n = 0; n < sizeof(cartao_DLIN) / sizeof(strCAMPO); n++)
    {
        ...
        campo = (txt.SubString(ind, count)).Trim();
        if (campo.IsEmpty())
            campo = cartao_DLIN[n].val_default;
        strList->Add(campo);
    }
}
