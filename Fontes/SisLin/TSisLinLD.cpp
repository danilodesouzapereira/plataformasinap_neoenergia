...
static int __fastcall EscolheLinhaV2(void *obj1,
   ...
   elem1 = (MatrixElementLD *) obj1;
   elem2 = (MatrixElementLD *) obj2;
...
VTSisLin * NewObjSisLinLD(int        num_linhas_submatriz,
   ...
   return(new TSisLinLD(num_linhas_submatriz, num_total_linhas, tCPU, nome_matriz));
   }

//---------------------------------------------------------------------------
__fastcall TSisLinLD::TSisLinLD(int        num_linhas_submatriz,
                                int        num_total_linhas,
                                VTTempoCPU *tCPU,
                                AnsiString nome_matriz)
                     :TSisLin(tCPU)
{
// Inicialização
Matriz     = new TControleMatrizLD(num_linhas_submatriz, num_total_linhas, nome_matriz);
...
//---------------------------------------------------------------------------
__fastcall TSisLinLD::~TSisLinLD(void)
{
delete Matriz;
}

//---------------------------------------------------------------------------
void __fastcall TSisLinLD::AlocaMemoriaTriang(void)
{
Matriz->AlocaMemoriaTriang();
}

//---------------------------------------------------------------------------
bool __fastcall TSisLinLD::Back(long double termo_conhecido[])
{
...
LDBack(termo_conhecido);
return(true);
}

//---------------------------------------------------------------------------
int __fastcall TSisLinLD::ColunaAProcessar(int ind_coluna_inic)
{
...
//---------------------------------------------------------------------------
bool __fastcall TSisLinLD::ColunaInversa(int ncol, long double vetor[])
{
...
//---------------------------------------------------------------------------
void __fastcall TSisLinLD::ColunasExtremas(int linha,
                                           int *menor_coluna,
                                           int *maior_coluna)
{
// Variáveis locais
MatrixElementLD *elem;
...
//---------------------------------------------------------------------------
VTSisLin * __fastcall TSisLinLD::Copia(AnsiString nome_copia,
                                       VTTempoCPU *tCPU2)
{
// Variáveis locais
int             num_linhas_submatriz, num_total_linhas;
clock_t         inicio;
TSisLinLD       *matriz_nova;
MatrixElementLD *elem;
...
matriz_nova = new TSisLinLD(num_linhas_submatriz, num_total_linhas, tCPU2, nome_copia);
...
//---------------------------------------------------------------------------
void __fastcall TSisLinLD::CopiaLinhaParaVetor(int             npa,
                                               int             linha,
                                               int             num_colunas,
                                               bool            *vet_processada,
                                               TSisLinLD       *mat2,
...
//---------------------------------------------------------------------------
VTSisLin * __fastcall TSisLinLD::CopiaMatrizParaLD(AnsiString nome_copia,
                                                   VTTempoCPU *tCPU2)
{
// Cópia normal long double -> long double
return(Copia(nome_copia, tCPU2));
}

//---------------------------------------------------------------------------
MatrixElementLD * __fastcall TSisLinLD::CriaElemento(int         linha,
                                                     int         coluna,
                                                     long double valor)
   {
   TBlocoLD        *bloco_atual;
   MatrixElementLD *pelem;

   // Obtém bloco atual
   if(lisBLOCOS->Count == 0) // Não foi alocado nenhum bloco ainda
      {
      bloco_atual = new TBlocoLD(Matriz->tamanho_bloco);
      lisBLOCOS->Add(bloco_atual);
      }
   else // Bloco atual é o último na lista
      {
      bloco_atual = (TBlocoLD *) lisBLOCOS->Last();
      }
...
//---------------------------------------------------------------------------
bool __fastcall TSisLinLD::CriaElementosDiagonal(void)
{
// Variáveis locais
MatrixElementLD *elem;
...
for(int linha=0; linha < Matriz->num_total_rows; linha++)
   ...
   elem = CriaElemento(linha, linha, Lzero);
   if(elem == NULL)
      {
...
//---------------------------------------------------------------------------
void __fastcall TSisLinLD::DefineIdentidade(void)
{
// Zera a matriz
ZeraMatriz();
...
for(int n=0; n < Matriz->num_total_rows; n++)
   {
   InsereElementoDiagonal(n, Lum);
   }
}

//---------------------------------------------------------------------------
int __fastcall TSisLinLD::ElementoLC(int         linha,
                                     int         coluna,
                                     long double *valor)
...
//---------------------------------------------------------------------------
int __fastcall TSisLinLD::ElementoOrdem(int         linha,
                                        int         ordem,
                                        long double *valor)
...
//---------------------------------------------------------------------------
TList * __fastcall TSisLinLD::ElementosLinha(int linha)
{
// Variáveis locais
MatrixElementLD *elem;
TElemLD         *elem_aux;
...
for(elem=Matriz->vet_diag[linha]; (elem != NULL); elem=elem->next_col)
   {
   elem_aux = new TElemLD(elem->column, elem->value);
...
//---------------------------------------------------------------------------
TList * __fastcall TSisLinLD::ElementosTriangSupLinha(int linha)
{
// Variáveis locais
MatrixElementLD *elem;
TElemLD         *elem_aux;
...
//---------------------------------------------------------------------------
MatrixElementLD * __fastcall TSisLinLD::ExisteElementoColuna(int linha,
                                                             int coluna)
{
// Variáveis locais
MatrixElementLD *elem;
...
//---------------------------------------------------------------------------
void __fastcall TSisLinLD::Imprime(AnsiString nome_arquivo,
                                   int        chave)
{
// Variáveis locais
FILE *arq;
...
//---------------------------------------------------------------------------
void __fastcall TSisLinLD::ImprimeVetorEsparso(AnsiString nome_vetor,
                                               TList      *lis)
{
// Variáveis locais
...
//---------------------------------------------------------------------------
bool __fastcall TSisLinLD::Insere(int         linha,
                                  int         coluna,
                                  long double valor,
                                  double      zero_virtual)
{
LDInsere(false, linha, coluna, valor, zero_virtual);
return(Erro == slSemErro);
}

//---------------------------------------------------------------------------
bool __fastcall TSisLinLD::InsereDiagonal(int         linha,
                                          long double valor,
                                          double      zero_virtual)
{
LDInsere(false, linha, linha, valor, zero_virtual);
return(Erro == slSemErro);
}

//---------------------------------------------------------------------------
void __fastcall TSisLinLD::InsereElementoDiagonal(int         linha,
                                                  long double valor)
{
// Adiciona valor
...
//---------------------------------------------------------------------------
void __fastcall TSisLinLD::InsereElementoFD(int         linha,
                                            int         coluna,
                                            long double valor,
                                            bool        flag_original)
{
...
//---------------------------------------------------------------------------
void __fastcall TSisLinLD::InsereLinha(int         lin,
                                       long double *vetor,
                                       double      zero_virtual)
{
// Variáveis locais
...
for(int col=0; col < num_linhas; col++)
   ...
   valor = vetor[col];
   if(fabsl(valor) < zero_virtual) continue; // Não existe o elemento (lin, col)
...
//---------------------------------------------------------------------------
bool __fastcall TSisLinLD::InsereQuatro(int         linha,
                                        int         coluna,
                                        long double valor,
                                        double      zero_virtual)
{
...
LDInsere(false, coluna, coluna, valor, zero_virtual);
if(Erro != slSemErro) return(false);

LDInsere(true, linha, coluna, (-valor), zero_virtual);
return(Erro == slSemErro);
}

//---------------------------------------------------------------------------
bool __fastcall TSisLinLD::InsereSimetrico(int         linha,
                                           int         coluna,
                                           long double valor,
                                           double      zero_virtual)
{
...
//---------------------------------------------------------------------------
VTSisLin * __fastcall TSisLinLD::Inverte(AnsiString nome_inversa,
                                         VTTempoCPU *tCPU2)
{
// Variáveis locais
bool        retorno = false;
long double *vetor = NULL;
clock_t     inicio;
...
matriz_inversa = NewObjSisLinLD(0, Matriz->num_total_rows, tCPU2, nome_inversa);
...
if(retorno)
   {
   vetor = new long double [Matriz->num_submatrix_rows];
...
//---------------------------------------------------------------------------
void __fastcall TSisLinLD::LDBack(long double vector_solution[])
{
// Local variables
int             column, init, row;
long double     *pd1;
...
//---------------------------------------------------------------------------
void __fastcall TSisLinLD::LDDefine(bool        flag_insere_simetrico,
                                    int         row,
                                    int         column,
                                    long double value,
                                    double      zero_virtual,
...
//---------------------------------------------------------------------------
void __fastcall TSisLinLD::LDInsere(bool        flag_insere_simetrico,
                                    int         row,
                                    int         column,
                                    long double value,
                                    double      zero_virtual)
...
if(fabsl(value) < zero_virtual) return;
...
//---------------------------------------------------------------------------
bool __fastcall TSisLinLD::LDTriang(void)
{
// Local variables
int             num_FD_novo, num_FD_prev, row_elim, row2;
long double     factor_elim, factor1, factor2;
clock_t         inicio;
MatrixElementLD *elem1, *elem2, *elem3, *elem4, *pdiag_elim;
...
for(int ind_linha_elim=0; ind_linha_elim < Matriz->num_factor_rows; ind_linha_elim++)
   ...
   factor_elim       = Lum / pdiag_elim->value;
...
//---------------------------------------------------------------------------
void __fastcall TSisLinLD::LimpaVetor(int     num_elems,
                                      TElemLD **vetor)
{
...
//---------------------------------------------------------------------------
int __fastcall TSisLinLD::LinhaAEliminar(int ind_linha_elim)
{
// Variáveis locais
int linha;
...
//---------------------------------------------------------------------------
bool __fastcall TSisLinLD::MatrizOK(void)
{
// Variáveis locais
int             num_elem, num_teste;
// int             erro;
MatrixElementLD *elem, *elem_prev;
...
//---------------------------------------------------------------------------
void __fastcall TSisLinLD::MenorMaior(double *menor,
                                      double *maior,
                                      double zero_virtual)
{
// Variáveis locais
...
//---------------------------------------------------------------------------
void __fastcall TSisLinLD::MontaVetorIdro(int *vet_idro)
{
...
//---------------------------------------------------------------------------
int __fastcall TSisLinLD::ObtemColuna_MAX_DIAG(int ind_coluna_inic)
{
// Esta função serve à decomposição QR
...
diag_esc       = fabsl(Matriz->vet_diag[coluna_esc]->value);
...
for(int ind_coluna=(ind_coluna_inic+1); ind_coluna < Matriz->num_submatrix_rows;
   ...
   diag   = fabsl(Matriz->vet_diag[coluna]->value);
...
//---------------------------------------------------------------------------
int __fastcall TSisLinLD::ObtemColuna_MIN_NLIG_MAX_DIAG(int ind_coluna_inic)
{
// Esta função serve à decomposição QR
...
diag_esc       = fabsl(elem_diag->value);
...
//---------------------------------------------------------------------------
int __fastcall TSisLinLD::ObtemLinha_MAX_DIAG(int ind_linha_elim)
{
// Variáveis locais
int             linha, nl_max, num_linhas;
long double     val_max;
MatrixElementLD *pdiag;
...
for(int ng=0; ng <= Matriz->ultimo_grupo; ng++)
   ...
   for(int nl=0; nl < num_linhas; nl++)
      ...
      if(fabsl(pdiag->value) > val_max)
         {
         nl_max    = nl;
         val_max   = fabsl(pdiag->value);
         grupo_max = grupo;
         }
      } // for(nl)
   } // for(ng)
...
//---------------------------------------------------------------------------
int __fastcall TSisLinLD::ObtemLinha_MIN_NLIG(int ind_linha_elim)
{
// Variáveis locais
int             linha, nl_inic;
MatrixElementLD *pdiag;
...
for(int ng=0; ng <= Matriz->ultimo_grupo; ng++)
   ...
   for(int nl=nl_inic; nl >= 0; nl--)
      ...
      linha = grupo->Linha(nl);
      pdiag = Matriz->vet_diag[linha];
      if(fabsl(pdiag->value) > Matriz->zerov)              // Achou pivô não nulo
         {
...
//---------------------------------------------------------------------------
int __fastcall TSisLinLD::ObtemLinha_MIN_NLIG_MAX_DIAG(int ind_linha_elim)
{
// Variáveis locais
int             linha, nl_max, num_linhas;
long double     val_max;
...
//---------------------------------------------------------------------------
int __fastcall TSisLinLD::ObtemLinha_SEM_ORDENACAO(int ind_linha_elim)
{
// Variáveis locais
int             linha;
MatrixElementLD *pdiag;
...
//---------------------------------------------------------------------------
void __fastcall TSisLinLD::OrdenaLinhas(TList *lisELEM_ZERAR,
                                        int   *vet_idro)
{
// Variáveis locais
// clock_t inicio_04;
...
//---------------------------------------------------------------------------
void __fastcall TSisLinLD::OrdenaLinhasMenorNumElemFD(TList *lisELEM_ZERAR)
{
...
for(int ind_elem=0; ind_elem < lisELEM_ZERAR->Count; ind_elem++)
   {
   elem                 = (MatrixElementLD *) lisELEM_ZERAR->Items[ind_elem];
   linha                = elem->row;
...
//---------------------------------------------------------------------------
void __fastcall TSisLinLD::OrdenaLinhasMenorUltimaColuna(int   *vet_idro,
                                                         TList *lisELEM_ZERAR)
{
...
//---------------------------------------------------------------------------
void __fastcall TSisLinLD::PermutaVetor(int         num_elems,
                                        long double vetor[])
{
...
//---------------------------------------------------------------------------
long double __fastcall TSisLinLD::ProdutoColunaVetor(int         coluna,
                                                     long double vetor[])
{
bool             cond;
long double      soma;
...
// Proteções
if(Erro != slSemErro) return(Lzero);  // Erro anterior
...
//---------------------------------------------------------------------------
long double __fastcall TSisLinLD::ProdutoLinhaVetor(int         linha,
                                                    long double vetor[],
                                                    int         tipo,
                                                    int         coluna)
/*
...
//---------------------------------------------------------------------------
VTSisLin * __fastcall TSisLinLD::ProdutoMatDiagMat(long double matriz_diag[],
...
// Faz um cast para obter objeto TSisLinLD (para acessar dados private)
matriz_prod2 = (TSisLinLD *) matriz_prod;
...
//---------------------------------------------------------------------------
void __fastcall TSisLinLD::ProdutoMatEscalar(long double valor,
                                             double      zero_virtual)
{
// Variáveis locais
MatrixElementLD *elem;
...
//---------------------------------------------------------------------------
VTSisLin * __fastcall TSisLinLD::ProdutoMatMat(VTSisLin   *matB,
                                               AnsiString nome_produto,
                                               VTTempoCPU *tCPU2)
{
...
// Variáveis locais
int             col_A, col_B, lin_B, num_cols_A, num_cols_B, num_linhas;
long double     valor, *vet_linha;
clock_t         inicio;
MatrixElementLD *elem_A, *elem_B;
TSisLinLD       *matB2, *matriz_prod2;
VTSisLin        *matriz_prod;
...
matriz_prod = NewObjSisLinLD(0, num_linhas, tCPU2, nome_produto);
if(matriz_prod == NULL) return(NULL);
matriz_prod2 = (TSisLinLD *) matriz_prod;  // Cast para acesso a dados private

// Início da contagem de tempo
inicio = clock();

// Aloca vetor para salvar valores de uma linha completa da matriz produto
vet_linha = new long double [num_linhas];
...
//---------------------------------------------------------------------------
VTSisLin * __fastcall TSisLinLD::ProdutoMatMatDiag(long double matriz_diag[],
                                                   AnsiString  nome_produto,
                                                   VTTempoCPU  *tCPU2,
                                                   bool        sobreescreve)
{
...
//---------------------------------------------------------------------------
VTSisLin * __fastcall TSisLinLD::ProdutoMatTranspMatDiag(long double matriz_diag[],
                                                         AnsiString  nome_produto,
                                                         VTTempoCPU  *tCPU2)
{
// Variáveis locais
...
// Cria nova matriz e seu cast para TSisLinLD
num_total_linhas = Matriz->num_total_rows;
matriz_prod      = NewObjSisLinLD(0, num_total_linhas, tCPU2, nome_produto);
matriz_prod2     = (TSisLinLD *) matriz_prod;
...
//---------------------------------------------------------------------------
bool __fastcall TSisLinLD::ProdutoMatTranspVetor(long double *vetor_ent,
...
//---------------------------------------------------------------------------
bool __fastcall TSisLinLD::ProdutoMatVetor(long double vetor_ent[],
                                           long double vetor_sai[],
                                           int         tipo,
                                           int         coluna)
{
...
//---------------------------------------------------------------------------
bool __fastcall TSisLinLD::QR_Back(long double termo_conhecido[],
                                   long double val_max)
{
// Variáveis locais
int             coluna;
long double     soma, valor_inverso;
clock_t         inicio;
MatrixElementLD *elem, *elem_diag;
...
for(int ncol=(Matriz->num_submatrix_rows-1); ncol >= 0; ncol--)
   ...
   elem_diag = Matriz->vet_diag[coluna];
   if(fabsl(elem_diag->value) > ZERO_VIRTUAL_QR) valor_inverso = Lum / elem_diag->value;
   else                                          valor_inverso = Lzero;
   ...
   soma *= valor_inverso;
   if(fabsl(soma) > val_max) soma = Lzero;
...
//---------------------------------------------------------------------------
VTSisLin * __fastcall TSisLinLD::QR_FastGivens(VTSisLin    **R,
                                               long double *vet_TC,
                                               bool        gera_matQt,
                                               VTTempoCPU  *tCPU2,
                                               AnsiString  nome_arq)
...
long double     abs_FD, alfa, beta, gama, gamam1, soma1, soma2;
long double     *vetD;
FILE            *file1;
clock_t         inicio;
TList           *lisREMOVE;
MatrixElementLD *elem, *elem1, *elem2, *elem_diag, **vet_lin_coluna,
                **vet_lin_linha;
TSisLinLD       *matAux2, *matMt2, *matT2;
VTSisLin        *matMt, *matT;
// Inspeção
...
if(gera_matQt)
   ...
   matMt         = NewObjSisLinLD(0, num_linhas, tCPU2, "MatrizMt");
   matMt2        = (TSisLinLD *) matMt;
...
vet_lin_coluna = new MatrixElementLD * [num_linhas];
vet_lin_linha  = new MatrixElementLD * [num_linhas];
vetD           = new long double       [num_linhas]; // Matriz diagonal [D]
for(int n=0; n < num_linhas; n++)
   {
   vet_processada[n] = false;
   vet_lin_coluna[n] = NULL;
   vet_lin_linha[n]  = NULL;
   vetD[n]           = Lum;  // Valor inicial: matriz identidade
   }
      ...
      for(int npa=0; npa < num_passagens; npa++)
         ...
         for(int ind_coluna2=ind_inic; ind_coluna2 < num_colunas_aux; ind_coluna2++)
            ...
            if(elem1 != NULL)
               ...
               {
               if(fabsl(soma1) > ZERO_VIRTUAL_QR)
            ...
            if(elem2 != NULL)
              ...
              vet_lin_linha[coluna2] = NULL;  // Zera o vetor
               ...
               {
               if(fabsl(soma2) > ZERO_VIRTUAL_QR)
...
//---------------------------------------------------------------------------
VTSisLin * __fastcall TSisLinLD::QR_Givens(VTSisLin    **R,
                                           long double *vet_TC,
                                           bool        gera_matQt,
                                           VTTempoCPU  *tCPU2,
                                           AnsiString  nome_arq)
...
if(gera_matQt)
   {
   num_passagens = 2;  // Montará as matrizes [R] e [Q]t
   matQt         = NewObjSisLinLD(0, num_linhas, tCPU2, "MatrizQt");
   matQt2        = (TSisLinLD *) matQt;
...
vet_coluna              = new MatrixElementLD * [num_linhas];
...
for(int ind_coluna=0; ind_coluna < num_colunas; ind_coluna++)
   ...
   abs_diag  = fabsl(elem_diag->value);
   ...
   for(int ind_elem=0; ind_elem < lisELEM_ZERAR->Count; ind_elem++)
      {
      elem_col = (MatrixElementLD *) lisELEM_ZERAR->Items[ind_elem];
      abs_FD   = fabsl(elem_col->value);
      if(abs_FD < ZERO_VIRTUAL_QR) continue; // Elemento FD já é zero
      ...
      if(abs_FD > abs_diag)
         ...
         senx  = Lum / sqrt(Lum + (soma1 * soma1));
         cosx  = senx * soma1;
         }
      else
         {
         soma1 = - elem_col->value / elem_diag->value;
         cosx  = Lum / sqrt(Lum + (soma1 * soma1));
         senx  = cosx * soma1;
         }
      ...
      for(int npa=0; npa < num_passagens; npa++)
         ...
         for(elem_lin=elem_lin_inic; (elem_lin != NULL); elem_lin=elem_lin->next_col)
            ...
            if(elem_col != NULL)  // "Dupla" (caso predominante)
               ...
               if(fabsl(soma1) > ZERO_VIRTUAL_QR) // Esta verificação é fundamental!
         ...
         for(int ne=0; ne < num_coluna; ne++)
            ...
            if(fabsl(pat1->valor_lin) > ZERO_VIRTUAL_QR) // Esta verificação é fundamental!
...
//---------------------------------------------------------------------------
VTSisLin * __fastcall TSisLinLD::QR_Householder(VTSisLin    **R,
                                                long double *vet_TC,
                                                bool        gera_matQt,
                                                VTTempoCPU  *tCPU2,
                                                AnsiString  nome_arq)
...
int             col, coluna, lin, linha, num_colunas, num_linhas,
                ultima_linha;
long double     beta, delta, norm2Vq, norm2X, soma, v1, valor;
FILE            *file1;
clock_t         inicio01, inicio02, inicio03, inicio04, inicio05, inicio06,
                inicio07;
TList           *lisREMOVE, *lisV, *lisW, *lisY;
MatrixElementLD *elem, **vet_linha;
TElemLD         *elemV, *elemV_diag, *elemW, *elemY, **vetV, **vetW;
TSisLinLD       *matH2, *matP2, *matR2;
...
if(gera_matQt)
   {
   matH  = NewObjSisLinLD(num_colunas, num_linhas, tCPU2, "MatrizH");
   matH2 = (TSisLinLD *) matH;
   }
...
vet_processada = new bool              [num_linhas]; // TEM que ser 'num_linhas'
vetV           = new TElemLD *         [num_linhas]; // Vetor V (cheio)
vetW           = new TElemLD *         [num_linhas]; // Vetor W (cheio)
vet_linha      = new MatrixElementLD * [num_linhas];
...
for(int ind_coluna=0; ind_coluna < num_colunas; ind_coluna++)
   ...
   for(elem=matR2->Matriz->vet_diag[coluna]; (elem != NULL); elem=elem->next_row)
      ...
      if(linha != coluna)
         {
         lisREMOVE->Add(elem);                              // Elemento será removido
         if(fabsl(elem->value) < ZERO_VIRTUAL_QR) continue; // Nada mais a fazer
         }
      // Cria elemento do vetor V
      elemV = new TElemLD(linha, elem->value);
   ...
   if(fabsl(norm2Vq) < ZERO_VIRTUAL_QR)
      {
      sucesso = false;
      break;
      }
   ...
   for(linha=0; linha < num_colunas; linha++) // NB: matriz [R]t - transposta!
      ...
      if(fabsl(soma) < ZERO_VIRTUAL_QR) continue; // Elemento de W existe mas é nulo
      elemW = new TElemLD(linha, soma);
      lisW->Add(elemW);
      }
   // Acumula tempo de processamento
   ...
   */
   ...
   for(int iv=0; iv < lisV->Count; iv++)
      ...
      for(elem=matR2->Matriz->vet_diag[linha]; (elem != NULL);
         ...
         if(elemW == NULL)
            {
            elemW = new TElemLD(elem->column, valor);
   ...
   for(int lin=0; lin <= ultima_linha; lin++)
      ...
      // if(fabsl(elemW->valor) < ZERO_VIRTUAL_QR) continue;
      lisW->Add(elemW);
      }
   // Acumula tempo de processamento
   AcumulaTempoCPU(tCPU2, ttZ03, inicio03);
   ...
   if(vet_TC != NULL)                        // Inicialização
      {
      soma = Lzero;  // Conterá o escalar (Vt * [TC])
      for(int iv=0; iv < lisV->Count; iv++)
         {
         elemV  = (TElemLD *) lisV->Items[iv];
         lin    = elemV->coluna;
   ...
   for(int iv=0; iv < lisV->Count; iv++)
      ...
      for(int jw=0; jw < lisW->Count; jw++)
         {
         elemW = (TElemLD *) lisW->Items[jw];
         col   = elemW->coluna;
         valor = v1 * elemW->valor;
         matR2->LDInsere(false, lin, col, valor, ZERO_VIRTUAL_QR);
         }
      */
      ...
      for(int jw=0; jw < lisW->Count; jw++)
         ...
         col   = elemW->coluna;
         valor = v1 * elemW->valor;
         // if(fabsl(valor) < ZERO_VIRTUAL_QR) continue;
         elem  = vet_linha[col];
         if(elem == NULL)
            {
      ...
      if(gera_matQt)
         ...
         for(int jv=iv; jv < lisV->Count; jv++) // NB: triâng. sup. (matriz simétrica)
            {
            elemV = (TElemLD *) lisV->Items[jv];
            col   = elemV->coluna;
            valor = v1 * elemV->valor;
            matH2->LDInsere(true, lin, col, valor, ZERO_VIRTUAL_QR); // Insere h(i,j) e h(j,i)
            }
         }
   ...
   if(gera_matQt)
      ...
      if(ind_coluna == 0)  // Primeira coluna: inicializa [P] = [H]
         ...
         for(linha=0; linha < num_linhas; linha++)
            ...
            if(fabsl(soma) < ZERO_VIRTUAL_QR) continue; // Elemento de Y existe mas é nulo
            elemY = new TElemLD(linha, soma);
            lisY->Add(elemY);
            }
         ...
         for(int iv=0; iv < lisV->Count; iv++)
            ...
            for(int jy=0; jy < lisY->Count; jy++)
               {
               elemY = (TElemLD *) lisY->Items[jy];
               col   = elemY->coluna;
               valor = v1 * elemY->valor;
               matP2->LDInsere(false, lin, col, valor, ZERO_VIRTUAL_QR);
               }
            }
         } // if(ind_coluna)
      // Acumula tempo de processamento
   ...
   for(int n=0; n < lisREMOVE->Count; n++)
      {
      elem = (MatrixElementLD *) lisREMOVE->Items[n];
...
//---------------------------------------------------------------------------
void __fastcall TSisLinLD::RemoveElementoDaColuna(MatrixElementLD *elem)
{
// Variáveis locais
MatrixElementLD *next, *prev;
...
//---------------------------------------------------------------------------
void __fastcall TSisLinLD::RemoveElementoDaLinha(MatrixElementLD *elem)
{
// Variáveis locais
MatrixElementLD *next, *prev;
...
//---------------------------------------------------------------------------
void __fastcall TSisLinLD::RemoveElementos(double zero_virtual,
                                           TList  *lisREMOVE)
{
// Variáveis locais
TList           *lisREMOVE_AUX;
...
if(lisREMOVE_AUX == NULL)
   ...
   for(int nl=0; nl < Matriz->num_total_rows; nl++)
      ...
      for(elem=Matriz->vet_diag[nl]->next_col; (elem != NULL); elem=elem->next_col)
         {
         if(fabsl(elem->value) < zero_virtual) lisREMOVE_AUX->Add(elem);
         }
      }
   }
...
for(int ne=0; ne < lisREMOVE_AUX->Count; ne++)
   {
   elem = (MatrixElementLD *) lisREMOVE_AUX->Items[ne];
...
//---------------------------------------------------------------------------
void __fastcall TSisLinLD::SomatoriaLinha(int         linha,
                                          long double *soma)
...
//---------------------------------------------------------------------------
VTSisLin * __fastcall TSisLinLD::Transposta(AnsiString nome_transp,
                                            VTTempoCPU *tCPU2)
{
// Variáveis locais
int             linha, num_total_linhas;
...
// Cria nova matriz e seu cast para TSisLinLD
num_total_linhas = Matriz->num_total_rows;
matriz_transp    = NewObjSisLinLD(0, num_total_linhas, tCPU2, nome_transp);
matriz_transp2   = (TSisLinLD *) matriz_transp;
...
//---------------------------------------------------------------------------
bool __fastcall TSisLinLD::Triang(void)
{
// Variáveis locais
bool retorno;
...
//---------------------------------------------------------------------------
bool __fastcall TSisLinLD::ZeraMatriz(void)
{
...
// Reinicia objeto 'TControleMatrizLD'
if(Matriz != NULL) Matriz->ReiniciaLD();
...
//---------------------------------------------------------------------------
void __fastcall TSisLinLD::ZeraVetor(int         num_elems,
                                     long double *vetor)
{
for(int n=0; n < num_elems; n++)
   {
