...
VTBarra* __fastcall TPreMonta::ExisteInsereBarra(AnsiString codigo)
	...
	try{//verifica se a Barra já existe
		if ((barra = monta->ExisteBarra(codigo)) == NULL)
			...
			{
			if (plog) plog->Add("Erro na criação do ponto de acoplamento " + AnsiQuotedStr(codigo, '\''));
			barra = NULL;;
			}
	return(barra);
	}
...
TSe* __fastcall TPreMonta::ExisteInsereSe(sdi_TRAFO_AT_MT &sdi)
	...
	try{//verifica se existe a rede em lisETD
		if ((se = ExisteSe(sdi.CodSubestacao)) == NULL)
			{//cria um novo objeto Se e insere em lisSE
			se = NewObjSe(sdi.CodSubestacao);
			lisSE->Add(se);
			//cria Barra AT
			...
			//cria nova Rede
			se->InsereRedeSE(redeSE = InsereRede(sdi.CodSubestacao, redeETD, barra_at));
			}
		else
			...
			{
			if (plog) plog->Add("Erro na criação da subestação " + AnsiQuotedStr(sdi.CodTrafo, '\''));
			se = NULL;;
			}
	return(se);
	}
...
bool __fastcall TPreMonta::InsereCargaBT(sdi_CARGA_BT &sdi)
   ...
   int         fases_tag;
	...
	//cofigura Log
	if (plog) plog->DefineValor("Eqpto", sdi.CodConsumidorBT);
	try{//verifica se as fases estão definidas
		if (sdi.CodFases.IsEmpty())
			...
			fases_tag = fases->ExisteFase(sdi.CodFases) & faseABCN;
			}
		//determina Barra da Carga
		if ((barra  = monta->ExisteBarra(sdi.CodPontoAcoplamento)) == NULL)
			{
			if (plog) plog->Add("Não existe ponto de acoplamento " + AnsiQuotedStr(sdi.CodPontoAcoplamento, '\''));
			//cria uma nova Barra, sem coordenadas
			if ((barra = InsereBarra(sdi.CodPontoAcoplamento)) == NULL) return(true);
			}
		//acrescenta prefixo "BT_" ao código da Curva
		sdi.TipologiaCurvaCarga = "BT_" + sdi.TipologiaCurvaCarga;
		//determina curva típica da Carga
		if ((curva = curvas->ExisteCurvaTipicaCarga(sdi.TipologiaCurvaCarga)) == NULL)
			{//procura Curva de IP
			if ((curva = curvas->ExisteCurvaTipicaIP(sdi.TipologiaCurvaCarga)) == NULL)
				{
				if (plog) plog->Add("Não existe tipologia da curva de carga " + AnsiQuotedStr(sdi.TipologiaCurvaCarga, '\''));
				return(true);
				}
			}
		...
		str_carga.codigo       = sdi.CodConsumidorBT;
      //str_carga.extern_id    = "";
		//define classe de consumidor/energia
		str_carga.classe_cons    = consINDEFINIDO;
		str_carga.curva_tipica   = curvas->ExisteCurvaTipicaCarga(sdi.TipologiaCurvaCarga);
		str_carga.modelo.icte_pu = 1.;
		str_carga.modelo.scte_pu = 0.;
		str_carga.modelo.zcte_pu = 0.;
...
bool __fastcall TPreMonta::InsereCargaMT(sdi_CARGA_MT &sdi)
   ...
   int         fases_tag;
	...
	//cofigura Log
	if (plog) plog->DefineValor("Eqpto", sdi.CodConsumidorMT);
	try{//verifica se as fases estão definidas
		if (sdi.CodFases.IsEmpty())
			{//assume carga trifásica
		...
		if ((redeMT = ExisteRede(sdi.CodAlimentador)) == NULL)
			{
		...
		//acrescenta prefixo "MT_" ao código da Curva
		sdi.TipologiaCurvaCarga = "MT_" + sdi.TipologiaCurvaCarga;
		//determina curva típica da Carga
		if ((curva = curvas->ExisteCurvaTipicaCarga(sdi.TipologiaCurvaCarga)) == NULL)
			{
			if (plog) plog->Add("Não existe tipologia da curva de carga " + AnsiQuotedStr(sdi.TipologiaCurvaCarga, '\''));
		...
		str_carga.car_id       = FalsoId();
		str_carga.bar_id       = barra->Id;
		str_carga.codigo       = sdi.CodConsumidorMT;
      //str_carga.extern_id    = "";
		//define classe de consumidor/energia
		str_carga.classe_cons    = consINDEFINIDO;
		str_carga.modelo.icte_pu = 1.;
...
bool __fastcall TPreMonta::InsereChaveBT(sdi_CHAVE_BT &sdi)
   ...
   //configura Resumo
	if (plog) plog->DefineValor("Eqpto", sdi.CodChave);
	try{//verifica se as fases estão definidas
		if (sdi.CodFases.IsEmpty())
			{//assume carga trifásica
			if (plog) plog->Add("Fases não definidas");
		...
		//converte fases
		fases_tag = fases->ExisteFase(sdi.CodFases);
		//verifica se existe a Rede
		if ((redeBT = ExisteRede(sdi.CodTrafo)) == NULL)
			{
			if (plog) plog->Add("Não está associado a uma rede");
			return(true);
			}
		//determina Barras
		if ((barra1 = ExisteInsereBarra(sdi.CodPontoAcoplamento1)) == NULL) return(true);
		if ((barra2 = ExisteInsereBarra(sdi.CodPontoAcoplamento2)) == NULL) return(true);
		//verifica se as Barras são iguais
		if (barra1 == barra2)
			{
			if (plog) plog->Add("Pontos de acoplamento iguais");
			return(true);
			}
		/*
		...
		if ((ligacao = redeBT->ExisteLigacao(eqptoTRECHO, sdi.codigo_segmento)) != NULL)
			...
			if ((ligacao->pbarra1 == barra2)&&(ligacao->pbarra2 == barra1))
				...
				{
				if (plog) plog->Add("Associada a segmento com acoplamento diferente");
				}
			}
		*/
		...
		if (! sdi.codigo_segmento.IsEmpty())
			...
			if ((ligacao = redeMT->ExisteLigacao(eqptoTRECHO, sdi.codigo_segmento)) != NULL)
				...
				if ((ligacao->pbarra1 == barra2)&&(ligacao->pbarra2 == barra1))
					...
					{
					if (plog) plog->Add("Chave MT associada a segmento com acoplamento diferente");
					}
				}
			}
		*/
...
bool __fastcall TPreMonta::InsereCircuito(sdi_CIRCUITO &sdi)
	...
	//configura Log
	if (plog) plog->DefineValor("Eqpto", sdi.CodAlimentador);
	try{//insere Rede em VTMonta
		if (plog) plog->DefineValor("Rede", sdi.CodAlimentador);
		//determina Barra inicial
		if ((barra = ExisteInsereBarra(sdi.CodPontoAcoplamento)) == NULL) return(false);
		//define tensão da Barra inicial
		barra->vnom = sdi.Vnom_kv;
		//insere Suprimento
		if (InsereSuprimento(sdi.CodAlimentador, barra, sdi.Vope_pu) == NULL) return(false);
		//insere a Rede
		if ((redeMT = InsereRede(sdi.CodAlimentador, redePRI, barra)) == NULL) return(false);
		//insere RedeMT na Se
		if ((se = ExisteSe(sdi.CodSubestacao)) != NULL)
			{
...
bool __fastcall TPreMonta::InsereCondutor(sdi_CONDUTOR &sdi)
	...
	try{//preenche estrutura strARRANJO2
		...
		str_arranjo.cfg_arranjoZ0Z1.iadm_amp   = sdi.Imax_amp;
		str_arranjo.cfg_arranjoZ0Z1.z1.r       = sdi.Rohm_km;
		str_arranjo.cfg_arranjoZ0Z1.z1.x       = sdi.Xohm_km;
		...
		if (monta->InsereArranjo(str_arranjo) == NULL) return(true);
			...
			{
			if (plog) plog->Add("Erro na criação do condutor " + AnsiQuotedStr(sdi.CodCondutor, '\''));
			return(false);
			}
	return(true);
   }
...
bool __fastcall TPreMonta::InsereCoordenada(sdi_COORDENADA &sdi)
	...
	try{//verifica se existe a Barra
		if ((barra  = monta->ExisteBarra(sdi.codbase)) == NULL)
			{
			if (plog) plog->Add("Não existe ponto de acoplamento " + AnsiQuotedStr(sdi.codigo, '\''));
			//cria uma nova Barra, sem coordenadas
...
//-----------------------------------------------------------------------------
bool __fastcall TPreMonta::InsereCurvaTipicaBT(sdi_CURVA &sdi)
	{
	//variáveis locais
	bool            sucesso;
	strCURVA_TIPICA str_curva;

	try{//acrescenta prefixo "BT_" ao código da Curva
		sdi.CodCurva = "BT_" + sdi.CodCurva;
		//monta a curva típica
		if (! MontaCurvaTipica(sdi, str_curva)) return(false);
		//complementa atributo específico de Curva de Carga
		//str_curva.classe_tag = consNULO;
		//insere Curva típica em VTMonta
		sucesso = (monta->InsereCurvaTipicaCarga(str_curva) != NULL);
		}catch (Exception &e)
			{
			sucesso = false;
			}
...
//-----------------------------------------------------------------------------
bool __fastcall TPreMonta::InsereCurvaTipicaMT(sdi_CURVA &sdi)
	{
	//variáveis locais
	bool            sucesso;
	strCURVA_TIPICA str_curva;

	try{//acrescenta prefixo "MT_" ao código da Curva
		sdi.CodCurva = "MT_" + sdi.CodCurva;
		//monta a curva típica
		if (! MontaCurvaTipica(sdi, str_curva)) return(false);
		//complementa atributo específico de Curva de Carga
		//str_curva.classe_tag = consNULO;
...
//-----------------------------------------------------------------------------
bool __fastcall TPreMonta::InsereRamalBT(sdi_RAMAL &sdi)
	{
	//variáveis locais
	int         fases_tag;
	strTRECHO   str_tre;
   ...
   //configura Resumo
	if (plog) plog->DefineValor("Eqpto", sdi.CodRamal);
	try{//determina Barras
		if ((barra1 = ExisteInsereBarra(sdi.CodPontoAcoplamento1)) == NULL) return(true);
		if ((barra2 = ExisteInsereBarra(sdi.CodPontoAcoplamento2)) == NULL) return(true);
		...
		if ((arranjo = arranjos->ExisteArranjo(sdi.CodCondutor)) == NULL)
			{
			if (plog) plog->Add("Não existe o condutor " + AnsiQuotedStr(sdi.CodCondutor, '\''));
			return(true);
			}
		...
		//verifica se as fases do Trecho são iguais às fases do Arranjo
			...
			{
			if (plog) plog->Add("Fases inconsistentes com fases do condutor");
			return(true);
			}
		//verifica se as Barras são iguais
		if (barra1 == barra2)
      ...
      if ((str_tre.comp_km < 0.001)||(str_tre.comp_km > 0.200))
         {//define comprimento de 10 metros
			if (plog) plog->Add("Comprimento = " + DoubleToStr("%4.3f km", sdi.Comprimento_km));
         }
...
bool __fastcall TPreMonta::InsereReguladorMT(sdi_REGULADOR &sdi)
	...
	//configura Resumo
	if (plog) plog->DefineValor("Eqpto", sdi.CodRegulador);
	try{//determina fases do primário e do secundário
		fases_pri = fases->ExisteFase(sdi.CodFasesPri);
		fases_sec = fases->ExisteFase(sdi.CodFasesSec);
		//verifica se as fases do primário e do secundário estão definidas
		if (sdi.CodFasesPri.IsEmpty())
			{
			if (plog) plog->Add("Fases do primário indefinidas");
			return(true);
			}
		if (sdi.CodFasesSec.IsEmpty())
			{
		...
		if ((regulador = (VTRegulador*)redeMT->ExisteLigacao(eqptoREGULADOR, sdi.CodRegulador)) != NULL)
		...
		str_regulador.codigo        = sdi.CodRegulador;
		...
		str_regulador.modelo        = reguAUTO_UNI;//reguPADRAO;
...
bool __fastcall TPreMonta::InsereSegmentoBT(sdi_SEGMENTO_BT &sdi)
   ...
   //configura Resumo
	if (plog) plog->DefineValor("Eqpto", sdi.CodSegmento);
	try{//determina Barras
		if ((barra1 = ExisteInsereBarra(sdi.CodPontoAcoplamento1)) == NULL) return(true);
		if ((barra2 = ExisteInsereBarra(sdi.CodPontoAcoplamento2)) == NULL) return(true);
		//verifica se as fases estão definidas
		...
		str_tre.bar1_id        = barra1->Id;
		str_tre.bar2_id        = barra2->Id;
		str_tre.codigo         = sdi.CodSegmento;
      str_tre.extern_id      = "";
		str_tre.fases          = fases_tag;
		str_tre.comp_km        = sdi.Comprimento_km;
		//define parâmetros para esquemático
...
bool __fastcall TPreMonta::InsereTrafoAT_MT(sdi_TRAFO_AT_MT &sdi)
	...
	//configura Resumo
	if (plog) plog->DefineValor("Eqpto", sdi.CodTrafo);
	try{//verifica se existe a Se
		...
		//calcula impedância em pu
		r_pu = (sdi.PerdaTotal_perc - sdi.PerdaVazio_perc) * 0.01;
		x_pu = 0.05;
		//insere Trafo
		...
		str_trafo.codigo        = sdi.CodTrafo + "_" +  IntToStr(sdi.SeqBanco);
		str_trafo.extern_id     = "";
		...
		str_trafo.perda_ferro = sdi.PerdaVazio_perc;
		//primário
...
bool __fastcall TPreMonta::InsereTrafoMT_BT(sdi_TRAFO_MT_BT &sdi)
	...
	try{//determina Barras
		...
		if (IsDoubleZero(barra2->vnom)) barra2->vnom = sdi.Vnom_sec_kv;
		//senecessário, cria a Rede BT
		if ((redeBT = ExisteInsereRedeBT(sdi.CodTrafo, barra1)) == NULL) return(true);
		//determina fases do primário e do secundário
		fases_pri = fases->ExisteFase(sdi.CodFasesPri);
		fases_sec = fases->ExisteFase(sdi.CodFasesSec);
		//verifica se as fases do terciário foram definidas
		if (! sdi.CodFasesTer.IsEmpty())
			{//trafo monofásico a 5 fios: compõe fases do secundário
			fases_sec |= fases->ExisteFase(sdi.CodFasesTer);
			}
		//calcula impedância em pu
		r_pu = sdi.R_perc   * 0.01;
		x_pu = sdi.Xhl_perc * 0.01;
		...
		//verifica consistência entre perda em vazio e perda total
		if (sdi.PerdaVazio_perc > sdi.PerdaTotal_perc)
			{
			if (plog) plog->Add("Perda em vazio maior que perda total");
			return(true);
			}
		//insere Trafo
		str_trafo.trf_id        = FalsoId();
		...
		str_trafo.perda_ferro = sdi.PerdaVazio_perc;
		//primário
		//define tipo de ligacao e tensão do primário
		vnom_pri = redeMT->BarraInicial()->vnom;
		switch(fases->NumeroDeFases(fases_pri))
			{
			case 1:
				str_trafo.pri.ligacao = lgMONOFASICO;
				str_trafo.pri.vnom    = vnom_pri /sqrt(3.);
				break;
			case 2:
				str_trafo.pri.ligacao = lgMONOFASICO;
				str_trafo.pri.vnom    = vnom_pri;
				break;
			case 3:
				str_trafo.pri.ligacao = lgTRIANG;
				str_trafo.pri.vnom    = vnom_pri;
		...
		//secundário
		str_trafo.sec.vnom      = sdi.Vnom_sec_kv;
		str_trafo.sec.tap       = sdi.Vtap_pu * sdi.Vnom_sec_kv;
		str_trafo.sec.defasagem = 0;
		str_trafo.sec.zterra.r  = 0;
		str_trafo.sec.zterra.x  = 0;
...
bool __fastcall TPreMonta::MontaCurvaTipica(sdi_CURVA &sdi, strCURVA_TIPICA &str_curva)
   ...
   try{//inicia estrutura
		str_curva.id                 = FalsoId();
		str_curva.tipo               = curvaPFP_MDP;
		str_curva.unidade            = unidPU_MED;
		str_curva.codigo             = sdi.CodCurva;
      str_curva.classe_tag         = -1;
		str_curva.tipica             = true;
		if      (sdi.TipoDia.AnsiCompareIC("DU") == 0)  str_curva.dia = DIA_UTIL;
		else if (sdi.TipoDia.AnsiCompareIC("SA") == 0)  str_curva.dia = SABADO;
		else if (sdi.TipoDia.AnsiCompareIC("DO") == 0)  str_curva.dia = DOMINGO;
		else
			{
			if (plog) plog->Add("TipoDia com valor inválido: " + AnsiQuotedStr(sdi.TipoDia, '\''));
			return(false);
			}
		str_curva.energia_min_kwhmes = -1;
		str_curva.energia_max_kwhmes = -1;
		str_curva.num_pat            = 24;
		str_curva.num_valor          = nvCURVA_PFP_MDP;
		str_curva.status             = sttNOVO;
		//monta curva com valores em pu de média
		MontaValoresCurva_pu_media(sdi.P_kw, 96, p_pu, 24);
		//prenche pontos
		for (int hora = 0; hora < 24; hora++)
			{
			str_curva.patamar[hora].hm_ini.hora   = hora;
			str_curva.patamar[hora].hm_ini.minuto = 0;
			str_curva.patamar[hora].hm_fim.hora   = hora + 1;
			str_curva.patamar[hora].hm_fim.minuto = 0;
			str_curva.patamar[hora].valor[0]      = p_pu[hora];
			str_curva.patamar[hora].valor[1]      = fatpot;
			str_curva.patamar[hora].valor[2]      = 0.;
			str_curva.patamar[hora].valor[3]      = 0.;
			}
		}catch (Exception &e)
			{
			return(false);
...
//-----------------------------------------------------------------------------
bool __fastcall TPreMonta::MontaValoresCurva_pu_media(double *p_kw, int len_p_kw, double *p_pu, int len_p_pu)
	{
	//variáveis locais
	double total, media;

	//proteção
	if ((len_p_kw != 96)||(len_p_pu != 24)) return(false);
	//calcula cada valor de p_pu como a média de 4 valores de p_kw
	for (int i = 0; i < 24; i++)
		{//soma valores de 4 valores de p_kw
		total =0.;
		for (int j = 0; j < 3; j++)
			{
			total += p_kw[(i*4)+j];
			}
		//salva média em p_pu (OBS: media ainda está em kW)
		p_pu[i] = total / 4.;
		}
	//calcula valor médio dos pontos em v_pu
	total =0.;
	for (int i = 0; i < 24; i++)
		{//soma valor do ponto
		total += p_pu[i];
	...
	media = total / 24;
	//proteção
	if (! IsDoubleZero(media))
		{
		//calcula pontos em v_pu em pu da média
		for (int i = 0; i < 24; i++)
			{
			p_pu[i] /= media;
			}
...
bool __fastcall TPreMonta::TrataIniciaDados(AnsiString cod_dominio)
	...
	if (codigo_dominio.IsEmpty()) codigo_dominio = DateToStr(DateOf(Now()));
	//inicia MontaRede
...
bool __fastcall TPreMonta::InsereArranjo(sdi_ARRANJO &sdi)
	...
	try{//preenche estrutura strARRANJO2
		...
		str_arranjo.fases      = fases->ExisteFase(sdi.CodFases);
		str_arranjo.res_terra  = 100.;
		str_arranjo.vmin_kv    = 0;
		str_arranjo.vmax_kv    = 0;
		str_arranjo.color      = clBlue;
...
VTCarga* __fastcall TPreMonta::InsereCargaIP(sdi_CARGA_IP &sdi)
   ...
   try{//verifica se as fases estão definidas
      if (sdi.CodFases.IsEmpty())
         ...
         fases_tag = fases->ExisteFase(sdi.CodFases) & faseABC;
         }
      //preenche estrutura strCARGA
...
bool __fastcall TPreMonta::InsereReguladorAutomatico(sdi_REGULADOR &sdi)
   ...
   try{//proteção: verifica se existe a Rede
      ...
      str_reg.fases       = fases->ExisteFase(sdi.CodFases);
      //proteção
      if (sdi.CodFases.IsEmpty())
