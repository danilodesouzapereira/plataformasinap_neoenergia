...
// ---------------------------------------------------------------------------
void __fastcall  TSinapChild::BeginEditorBlock(TObject *obj)
{
...
void __fastcall TSinapChild::DefineArranjos(VTApl *apl_ext)
{
	...
	{ // determina Arranjo externo
		...
		if ((ext.arranjo->suporte != NULL) && (ref.arranjo->suporte != NULL))
		{
			...
			{
				if ((ext.arranjo->fase[nf].pto_fixacao != NULL) &&
					(ext.arranjo->fase[nf].cabo != NULL))
				{
					ref.arranjo->fase[nf].pto_fixacao =
						ref.arranjo->suporte->ExistePtoFixacao
						(ext.arranjo->fase[nf].pto_fixacao->id);
					ref.arranjo->fase[nf].cabo =
						ref.arranjos->ExisteCabo(ext.arranjo->fase[nf].cabo->Id);
				}
			}
		}
	}
...
// ---------------------------------------------------------------------------
void __fastcall TSinapChild::DefineCorTipoChave(VTApl *apl_ext)
{ // DVK 2015.10.01
	// variáveis locais
	VTTipoChave *tipochave, *tipochave_ext;
	VTTipos *tipos_ext = (VTTipos*)apl_ext->GetObject(__classid(VTTipos));
	...
	ActionTopo->Execute();
	// loop em todos tipochave
	for (int nt = 0; nt < tipos->LisTipoChave()->Count; nt++)
	{
		tipochave = (VTTipoChave*)tipos->LisTipoChave()->Items[nt];
		tipochave_ext = tipos_ext->ExisteTipoChave(tipochave->Id);
		// atualiza cor do tipochave
		if (tipochave_ext)
			tipochave->Color = tipochave_ext->Color;
	}
}
...
void __fastcall TSinapChild::DefinePatamares(VTApl *apl_ext)
{
	...
	// proteção
	if (ref.patamares == NULL)
		return;
	if (ext.patamares == NULL)
		return;
...
// https://sinapsisenergia.teamwork.com/#/tasks/15933980
// ---------------------------------------------------------------------------
void __fastcall  TSinapChild::EndEditorBlock(TObject *obj)
{
	std::set<void *>::iterator it;
	
	//editorBlocked = false;
	it = semaforo.find(obj);
	if(it != semaforo.end())
	{
		semaforo.erase(it);
	}
...
void __fastcall TSinapChild::FormStkAdd(AnsiString caption, TActionList *ActionList, bool expand)
{
	...
	if (FormStk != NULL)
		FormStk->TreeViewAdd(caption, ActionList, expand);
}
...
void __fastcall TSinapChild::FormStkAdd(AnsiString caption, TList *LisActionList, bool expand)
{
	// insere ActionList em FormStk
	if (FormStk != NULL)
		FormStk->TreeViewAdd(caption, LisActionList, expand);
}
...
void __fastcall TSinapChild::FormStkRemove(TActionList *ActionList)
{
	...
	if (FormStk != NULL)
		FormStk->TreeViewRemove(ActionList);
}
...
void __fastcall TSinapChild::FormStkShow(AnsiString caption)
{
	...
	if (FormStk == NULL)
		FormStk = new TFormStk(apl->Form, apl, MultiPanel->Panel1);
	// atualiza Caption
	if (!caption.IsEmpty())
		FormStk->Caption = caption;
	// exibe TFormStk
	if (!FormStk->Visible)
		FormStk->Show();
}
// ---------------------------------------------------------------------------
void __fastcall TSinapChild::HabilitaBotoesEditor(bool habilitado)
{
	TControl *botao;
	TToolButton *but;

	for (int nb = 0; nb < lisBOTAOEDITOR->Count; nb++)
	{
		botao  = (TControl*) lisBOTAOEDITOR->Items[nb];
		botao->Enabled = habilitado;
		//se tiver propriedades de um toolbutton
		if (botao->InheritsFrom(__classid(TToolButton)))
		{
			but = (TToolButton*)botao;
			if(habilitado == false)
			{
				but->Down =  false;
			}
...
// https://sinapsisenergia.teamwork.com/#/tasks/15933980
// ---------------------------------------------------------------------------
void __fastcall  TSinapChild::HabilitaEditor(bool habilitado)
{
	if (!habilitado)
	{	//destroi forms de editor aberto
	...
	{
		ActionEditor->Enabled = habilitado;
	}
	//habilita/desabilita os botoes
	HabilitaBotoesEditor(habilitado);
}
// ---------------------------------------------------------------------------
void __fastcall  TSinapChild::InsereBotaoEditor(TControl *botao)
{
	if (botao != NULL)
	{
		if (lisBOTAOEDITOR->IndexOf(botao) < 0)
		{
            lisBOTAOEDITOR->Add(botao);
		}
	}
}
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
void __fastcall  TSinapChild::RemoveBotaoEditor(TControl *botao)
{
    lisBOTAOEDITOR->Remove(botao);
}
// ---------------------------------------------------------------------------
void __fastcall TSinapChild::TrataAlteracaoPatamares(int num_pat)
{
	...
	// redefine Eqptos das reduções das MNets
...
void __fastcall TSinapChild::TrataAlteracaoPatamaresEqpto(VTEqpto *eqpto, int num_pat)
{
	...
	else if (eqpto->TipoLigacao())
	{
		...
		{
			((VTRegulador*)eqpto)->Max_pat = num_pat;
		}
...
void __fastcall TSinapChild::TrataAlteracaoPatamaresEstudo(int num_pat)
{
	...
	// proteção
	if ((estudo == NULL) || (estudo->Id == estudoID_ZERO))
		return;
