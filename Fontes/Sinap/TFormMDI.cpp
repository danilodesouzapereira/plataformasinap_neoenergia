...
__fastcall TFormMDI::TFormMDI(TComponent *Owner) : TForm(Owner)
{
	...
	// inicia SinapToolKit
	SinapToolKitInicia();
	// retira Perda Regulatoria se a licença for da Copel
	RetiraPerdaRegulatoria();

	// apaga arquivos temporarios antigos
	DeleteTmp();
    //desabilita alguns módulos se a licença for SINAPviewer
    DesabilitaModulos();
}
...
void __fastcall TFormMDI::ActionAlertaExecute(TObject *Sender)
{
   ...
   // Inicialização (no código abaixo existe a possibilidade de InsereMsgAlerta()
   // ser chamada sem que a variável 'rede' tenha sido definida - mas é preciso
   // inicializá-la para que InsereMsgAlerta() escolha o alerta correto)
   // HPS, 2020.01.01
   rede = NULL;
...
//		return;
	...
	{ // verifica se o usuário deseja atualizar as redes abertas
		// FKM 2019.02.22 agora é gerenciado pelo form configura flow
...
/*
 ...
 {
 Aviso("Erro ao alocar licença: " + e.Message);
 }
 // reinicia controle de uso
 IniciaControleUso();
 // habilita/desabilita módulos de acordo c/ a Licenca alocada
...
void __fastcall TFormMDI::ActionLicencaAlocaRemotaExecute(TObject *Sender)
{
	...
	{ // cria objeto gestaoLicenca e form
		...
		{ // avisa o usuario que o Sinap deve ser reiniciado
			Aviso("O programa será reinicializado para efetivar a alocação da licença");
...
void __fastcall TFormMDI::ActionLicencaExibeExecute(TObject *Sender)
{
	...
	{
		Aviso("Erro ao apresentar dados da licença: " + e.Message);
	}
	// exibe logotipo
	LogoShow();
}
/*
 ...
 " não poderá ser executado nesta máquina após a liberação da Licença de Uso.",
 "Confirma liberação da Licença de Uso ?") != IDYES)
 return;
...
void __fastcall TFormMDI::ActionLicencaLiberaExecute(TObject *Sender)
{
	...
	if (Confirma("O programa " + produto->Codigo +
		" não poderá ser executado nesta máquina após a liberação da Licença de Uso.\n" +
		"A licença com login " + AnsiQuotedStr(licenca->Login, '\'') + " será liberada.\n" +
		"TODOS os arquivos que estiverem entre as pastas do " + produto->Codigo +
		" (incluindo redes) serão apagados durante a alocação da nova licença.",
		"Confirma liberação da Licença de Uso ?") != IDYES)
		return;
...
void __fastcall TFormMDI::ActionLoginOffExecute(TObject *Sender)
{
	...
	if (Confirma
		("Você está DESABILITANDO a proteção local de uso do programa.\n"
		"O uso do programa ficará liberado a todos usuários deste micro.", "Confirma esta opção ?")
		== IDYES)
...
void __fastcall TFormMDI::ActionLoginOnExecute(TObject *Sender)
{
	...
	// usuário está habilitando Login local
	if (Confirma
		("Você está HABILITANDO a proteção local de uso do programa.\n"
		"O uso do programa ficará restrito ao usuário da Licença de Uso.", "Confirma esta opção ?")
		== IDYES)
	{ // solicita identificação do usuário
		form = DLL_NewFormLogin(this, apl);
		if (form->ShowModal() == mrOk)
...
//		return;
	...
	LogoHide();
	try
	{ // cria um novo FormHistoricoPerdareg (que é do tipo FormChild)
...
//		return;
	...
	LogoHide();
	try
	{ // cria um novo FormHistoricoPerdatec (que é do tipo FormChild)
	...
	{ // salva rede
		...
		{ // verifica se foi definida uma msg p/ o usuário
			if (ActionSalva->Hint.IsEmpty())
				Aviso("A opção de salvar não está disponível.\nUtilize o comando 'Salvar como...' "
					);
			else
...
//				VerificaModulos();
		...
		{ // propoe configuracao de proxy e tenta denovo
			if (Confirma("Não foi possível liberar a licença.",
	...
	{
		Aviso("Não foi possível liberar a licença de uso: " + e.Message);
		// destroi objeto
		DestroiTObject(licencaBO);
	}
}
...
//		return (NULL);
	...
	{
		Aviso("Verifique que a instalação do programa e alocação de licenças foram realizadas corretamente\n");
		return (NULL);
	}
	catch (...)
	{
...
void __fastcall TFormMDI::RetiraPerdaRegulatoria(void)
{
	...
	{ // verifica se é da Copel
		...
		{
			menuitem = sinap_mdi->MainMenu->Items->Find("Perda Regulatória");
...
bool __fastcall TFormMDI::ValidaLicenca(bool show_diag)
{
	...
	// verifica se a licença é antiga (sem operações)
	if (!licencas->ValidaOperacoes(diagnostico))
	{ // exibe aviso de licença inválida
		Aviso(diagnostico);
		// libera a licença de uso
		LiberaLicenca();
		return (false);
	}
	if (show_diag)
...
void __fastcall TFormMDI::VerificaBaseDados(void)
{
	...
	try
	{ // determina última base de dados utilizada
		geral->Infoset->Clear();
