...
enum eqptoSTATUS {//status do Eqpto
				 ...
				 sttERROGIS       			=0x00000100, //eqpto importado de sistema externo com atributos errados/incompletos
                 //específico p/ Chave
				 sttCHAVE_CBR     			=0x00010000, //CBR
				 sttCHAVE_CBTL    			=0x00020000, //CBTL
				 sttCHAVE_LOCKOUT 			=0x00040000, //LOCkOUT
				 sttCHAVE_ABERTO_ORIGINAL 	=0x00080000, //Somente no planejamento, inicializado com o estado da rede original
                 //específico p/ Carga
				 sttCARGA_VIC     			=0x10000000,
                 //específico p/ Arranjo
				 sttRAMAL         			=0x10000000,
                 //específico p/ Rede
				 sttCBFSH         			=0x00010000,
				 //controle p/ destaque
				 sttDESTACADO     			=0x20000000,
                 //controle p/ seleção de Eqptos em telas
				 sttCHECKED      			=0x40000000, //1=selecionado, 0=não selecionado
...
enum eqptoSHOW {//atributos comuns a todos Eqptos
               ...
               //TrafoZZ-------------------------------
               //atributos de topologia
               trzz_S_MVA    =0x00000020,
               trzz_V_NOM    =0x00000040,
               //atributos de fluxo
               trzz_CARGTO   =0x00000080,    //carregamento(%) - não implementado
               //YREF----------------------------------
               //atributos de topologia
             ...
             //               =0x02000000,   //não usado
               ...
               reg_CFG        =0x20000000,   //ajuste automático ou fixo+vpu+tap
               //TRAFO--------------------------------
               //atributos de topologia
               ...
               tre_INOM       =0x10000000,   //corrente admissível
               tre_FASES      =0x20000000,   //fases do arranjo do trecho
               tre_ZSEQ_OHM   =0x40000000,   //Zseq0 e Zseq1 do trecho em ohm (OBS: pode não existir)
               tre_ZSEQ_PU    =0x80000000,   //Zseq0 e Zseq1 do trecho em pu (OBS: pode não existir)
               //
               //MUTUAS-------------------------------
               //atributos de topologia
...
//---------------------------------------------------------------------------
enum eLTC {ltcFIXO=0, ltcPRIMARIO, ltcSECUNDARIO};
...
class VTEqpto : public TObject
   ...
   public:  //property
      ...
      __property unsigned   Tag                       = {read=PM_GetTag,             write=PM_SetTag};
   ...
   public:
      ...
      virtual bool       __fastcall CoordenadasUtm_m(int &xm, int &ym, int &x1, int &y1,
                                                     int &x2, int &y2, int &x3, int &y3);
      virtual void       __fastcall CopiaAtributosDe(VTEqpto &ref);
   ...
   protected: //métodos acessados via property
      ...
      virtual unsigned   __fastcall PM_GetTag(void);
      ...
      virtual void       __fastcall PM_SetTag(unsigned tag);
   ...
   protected: //dados acessados via property
      struct   {
               ...
               unsigned   tag;     //variável auxiliar
               AnsiString codigo;
