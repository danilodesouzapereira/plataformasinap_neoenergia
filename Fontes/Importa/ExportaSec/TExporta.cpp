...
#include "..\..\InterfaceTXT\Exporta\VTExportaSecTXT.h"
#include "..\..\Log\VTLog.h"
...
//---------------------------------------------------------------------------
VTExportaSec* __fastcall NewObjExporta(VTApl *apl)
   {
...
bool __fastcall TExportaSec::Executa(bool showProgress)
   ...
   //exibe progresso
   if(showProgress) prog->Show("Exportar redes em arquivos TXT", 1, lisSEC->Count+1);
   ...
   for(int npri = 0; npri < lisPRI->Count; npri++)
      ...
      for (int n = 0; n < lisSEC->Count; n++)
         ...
         //incrementa
         if(showProgress) prog->NextPosition();
         if(showProgress) prog->SetSubTitulo(secundario->Rede->Codigo);
