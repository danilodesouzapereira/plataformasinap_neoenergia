...
// ---------------------------------------------------------------------------
__fastcall TFormEdtRegiao::TFormEdtRegiao(TComponent* Owner, VTApl *apl, TWinControl *parent,
	TAction *OnDone, TList* lisRegiao) : TFormEdt(Owner, parent)
{
	// variaveis locais
	VTRedegraf *redegraf;
	VTRegioes *regioes;
	...
	if (lisRegiao == NULL)
		listaRegioesEditaveis = regioes->LisRegiao();
	else
	{
		listaRegioesEditaveis = lisRegiao;
	}
...
__fastcall TFormEdtRegiao::~TFormEdtRegiao(void)
{
	...
	{
		delete listaRegioesNovas;
		listaRegioesNovas = NULL;
	}
}
...
{
	try
	{ // cria um novo objeto TEdtRegiao
		if (edtRegiao != NULL)
			delete edtRegiao;
...
{ // variávei locais
	...
	if ((nodeExclusao = tvRegiao->Selected) != NULL)
	{
		if (nodeExclusao->Level == 0)
		{
			...
			{
				RemoveRegiao(regiao);
				// remove o segmento conforme a regiao excluida
				RemoveSegmento(regiao);
			}
			// exclui o node do treeView do form
			nodeExclusao->Delete();
			// atualiza o gráfico
...
void __fastcall TFormEdtRegiao::ExcluiSegmentosDaRegiao(VTRegiao *regiao)
{
	...
	{
		delete lisSegmentos_excluir;
		lisSegmentos_excluir = NULL;
	}
}
...
// ---------------------------------------------------------------------------
void __fastcall TFormEdtRegiao::RemoveSegmento(VTRegiao *regiao)
{ // variaveis locais
	TList *lisSegmentos;
	VTSegmento *seg;
	...
	if (lisSegmentos != NULL)
	{
		for (int ns = (lisSegmentos->Count - 1); ns >= 0; ns--)
		{
			seg = (VTSegmento*) lisSegmentos->Items[ns];
			if (seg != NULL)
			{
				if (regiao == seg->Regiao)
				{
					// mercado->RetiraSegmento(seg);
					seg->Regiao = NULL;
				}
			}
		}
...
{ // variáveis locais
	...
	{ // limpa área de texto do TreeNode
		if (Node->Selected)
		{
			pCanvas->Brush->Color = clMenuHighlight;
		}
		pCanvas->FillRect(NodeRect);
		// determina retângulo p/ exibir cor associada à curva de crescimento
		TRect rect_cor(NodeRect.Left + 1, NodeRect.Top + 1,
...
// ---------------------------------------------------------------------------
void __fastcall TFormEdtRegiao::tvRegiaoEditing(TObject *Sender, TTreeNode *Node, bool &AllowEdit)
