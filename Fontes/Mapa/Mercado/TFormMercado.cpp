...
//#include "..\..\DLL_Inc\Cronometro.h" //added
...
__fastcall TFormMercado::~TFormMercado(void)
   ...
   if (lisCRE != NULL)
      {delete lisCRE; lisCRE = NULL;}
   if (lisREGIAO_pre!= NULL)
      {delete lisREGIAO_pre; lisREGIAO_pre = NULL;}
   //retorna o modo grafico
   RetornaModoGrafico();
   //destroi form
   if(formEdtRegiao!= NULL)
      {delete formEdtRegiao; formEdtRegiao = NULL;}
   }
...
void __fastcall TFormMercado::ActionCancelaExecute(TObject *Sender)
   ...
   if (ExisteAlteracao())
      ...
      if (Confirma("As alterações efetuadas serão perdidas.",
                   "Deseja descartar as alterações ?") != IDYES) return;
...
void __fastcall TFormMercado::ActionEditaSegmentoExecute(TObject *Sender)
   ...
   while (node_segmento->Level > 1) {node_segmento = node_segmento->Parent;}
   //verifica se há uma Segmento selecionada
   if ((segmento = ExisteSegmentoSelecionada(node_segmento)) == NULL) return;
   try{//cria FormEdtTaxa p/ edição do Crescimento
...
void __fastcall TFormMercado::ActionReiniciaTaxasExecute(TObject *Sender)
	...
	//pede confirmação do usuário
	if (Confirma("Todas as taxas de crescimento serão excluídas.",
					 "Confirma?") != IDYES) return;
...
void __fastcall TFormMercado::ActionRetiraSegmentoExecute(TObject *Sender)
   ...
   if ((node = TreeView->Selected) == NULL) return;
   ...
   //verifica se há uma Segmento selecionada
   if ((segmento = ExisteSegmentoSelecionada(node)) == NULL) return;
...
void __fastcall TFormMercado::CBoxAnoInicia(void)
   ...
   if (mercado->AnoInicial == 0) mercado->AnoInicial = ano_ini;
   if (mercado->AnoFinal   == 0) mercado->AnoFinal   = ano_fim;
   //define ano inicial e final
   if (ano_ini > mercado->AnoInicial) ano_ini = mercado->AnoInicial;
   if (ano_fim < mercado->AnoFinal  ) ano_fim = mercado->AnoFinal;
...
TColor __fastcall TFormMercado::ChartCorSerie(int ind)
   {
   TColor color[] = {clBlue, clRed,     clGreen,  clYellow,
                     clTeal, clFuchsia, clBlack, clLime};
...
//---------------------------------------------------------------------------
void __fastcall TFormMercado::ChartIniciaSerie(VTCrescimento *crescimento, TChartSeries *serie, TColor cor, bool first)
   {
   //variáveis locais
   double val;
...
VTCrescimento* __fastcall TFormMercado::ExisteCrescimentoSelecionado(TTreeNode *node)
   ...
   if (class_name.AnsiCompareIC("TCrescimento") == 0) crescimento = (VTCrescimento*)object;
   return(crescimento);
   }
...
VTSegmento* __fastcall TFormMercado::ExisteSegmentoSelecionada(TTreeNode *node)
   ...
   if ((object = (TObject*)node->Data) == NULL) {return(NULL);}
   class_name  = object->ClassName();
   if (class_name.AnsiCompareIC("TSegmento") == 0) segmento = (VTSegmento*)object;
   return(segmento);
   }
...
void __fastcall TFormMercado::RetiraDaBaseSegmentoSemCrescimento(void)
   {
	...
	for (int nr = lisSEGMENTO->Count-1; nr >= 0; nr--)
		...
		segmento = (VTSegmento*)lisSEGMENTO->Items[nr];
		//não elimina Segmento Default
		if (segmento->Tipo == smDEFAULT) continue;
...
//---------------------------------------------------------------------------
void __fastcall TFormMercado::RetiraDaBaseSegmentoSemRegiao(VTSegmento *segmento)
   {
   VTRegioes  *regioes = (VTRegioes*)apl->GetObject(__classid(VTRegioes));
   TList      *lisRegiao = regioes->LisRegiao();

   //não elimina Segmento Default
   if (segmento->Tipo == smDEFAULT) return;
   //retira Segmento da base de dados
   ...
   VTRegioes 	*regioes = (VTRegioes*)apl->GetObject(__classid(VTRegioes));
	...
	for (int nr = lisSEGMENTO->Count-1; nr >= 0; nr--)
		...
		if (segmento->Tipo == smREGIAO)
         ...
         RetiraDaBaseSegmentoSemRegiao(segmento);
         }
      }
   }
...
//---------------------------------------------------------------------------
void __fastcall TFormMercado::TreeViewAdvancedCustomDrawItem(
   ...
   if (Node->Level < 2)
      ...
      if (Node->Selected) {TreeView->Canvas->Brush->Color = clMenuHighlight;}
   ...
   //verifica TreNode Level
      ...
      //determina retângulo p/ exibir cor associada à curva de crescimento
		TRect rect_col1(NodeRect.Left+1,
                      NodeRect.Top+1,
      ...
      rect_col1.Right  = NodeRect.Right;
		...
		TreeView->Canvas->TextRect(rect_col2, rect_col2.Left, rect_col2.Top + 2, DoubleToStr("%6.2f", taxa_perc));
		}
	//restaura cor de Brush
	TreeView->Canvas->Brush->Color = color;
	//bloqueia alteração automática
...
//---------------------------------------------------------------------------
TTreeNode* __fastcall TFormMercado::TreeViewInsereSegmento(TTreeNode *node_pai, VTSegmento *segmento)
   {
   //variáveis locais
   ...
   //proteção:
	if ((segmento == NULL)||(node_pai == NULL)) return(NULL);
	//cria TreeNode p/ a Região
	node_segmento = TreeView->Items->AddChildObject(node_pai, segmento->Codigo, segmento);
   //verifica se as taxas de crescimento ainda não foram definidas
   if (! segmento->Enabled) return(node_segmento);
   ...
   for (int n = 0; n < lisEXT->Count; n++)
      ...
      node_classe = TreeView->Items->AddChildObject(node_segmento, crescimento->Codigo, crescimento);
...
bool __fastcall TFormMercado::VerificaSomenteSDBT(void)
   ...
   if(lisREDESCARREGADAS)
      {delete lisREDESCARREGADAS; lisREDESCARREGADAS = NULL;}
   return (sucesso);
   }
//---------------------------------------------------------------------------
