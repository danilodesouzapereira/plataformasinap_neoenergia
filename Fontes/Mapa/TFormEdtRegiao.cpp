...
__fastcall TFormEdtRegiao::~TFormEdtRegiao(void)
   ...
   //destroi lista
   if (listaRegioesNovas)	{delete listaRegioesNovas; listaRegioesNovas = NULL;}
	//RegioesVisible(false);
	}

//---------------------------------------------------------------------------
void __fastcall TFormEdtRegiao::ActionCriarRegiaoExecute(TObject *Sender)
   {
	int numNode;
...
//---------------------------------------------------------------------------
void __fastcall TFormEdtRegiao::ActionFinalizarRegiaoExecute(TObject *Sender)
	{
...
void __fastcall TFormEdtRegiao::ConfirmaEdicaoRegiao(void)
   ...
   VTGrafico *grafico = (VTGrafico*)apl->GetObject(__classid(VTGrafico));
	...
	if(VerificaMinPontos(regiao))
		...
		butFinaliza->Enabled = false;
		numVertices = 0;
      //adiciona a lista de regioes novas
      listaRegioesNovas->Add(regiao);
      regiao->Obj = lisRegiao;
      //reseta o ponteiro de regiao
		regiao = NULL;
		//segmento = NULL;
		nodeLastRegiao = NULL;
		//atualiza o gráfico
...
//	VTPoligono		*poligono;
//	TPoint			*ponto;
//	//TTreeNode		*nodeRegiao;
//	VTSegmento		*segmento;
//	VTRegiao			*regiao;
//	//int				nrsg; //numero de regioes sem segmento
//
//
//	//inicia as listas
...
//	//também adiciona os vertices do poligono dessa regiao
...
////				//contador meio global (solução ruim =( )
////				numVertices = 0;
////				for (int np = 0; np < (poligono->LisPoint())->Count; np++)
////				{
////					ponto = (TPoint*) (poligono->LisPoint())->Items[np];
////					InsereVerticeNoTreeView(ponto->X,ponto->Y);
////				}
//			}
//		}
//	}
...
//			//adiciona novo treenode com o nome da nova regiao
//			nodeLastRegiao = tvRegiao->Items->AddObject(NULL,segmento->Codigo, segmento);
//			//reinicia o contador do Form usado pra contar vertices
////			numVertices = 0;
////			poligono = regiao->Poligono;
////			//adiciona cada vertice do poligono ao form
////			for (int np = 0; np < (poligono->LisPoint())->Count; np++)
////			{
////				ponto = (TPoint*) (poligono->LisPoint())->Items[np];
////				InsereVerticeNoTreeView(ponto->X,ponto->Y);
...
void __fastcall TFormEdtRegiao::FormClose(TObject *Sender, TCloseAction &Action)
{
	...
	if(form_dono)form_dono->Show();
   //executa a action enviada
   if(onDone != NULL)onDone->Execute();
   Action = caFree;
   ModalResult = mrOk;
}
...
//---------------------------------------------------------------------------
void __fastcall TFormEdtRegiao::FormShow(TObject *Sender)
{
   VTGrafico   *grafico  = (VTGrafico*)apl->GetObject(__classid(VTGrafico));

   //mostra regioes editaveis
	//regioes->RegioesVisible(listaRegioesEditaveis, true);
   regioes->RegioesVisibleComplementar(listaRegioesEditaveis, true);
   //atualiza grafico
   grafico->Redraw();
...
//---------------------------------------------------------------------------
void __fastcall TFormEdtRegiao::Set_ActionOnDone(TAction *OnDone)
   {
