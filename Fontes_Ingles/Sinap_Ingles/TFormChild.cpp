...
//---------------------------------------------------------------------------
__fastcall TFormChild::TFormChild(TComponent *Owner, VTApl *apl_owner, TAction *ActionOnClose, int num_seq, int stk_option)
   : TForm(Owner)
	{
	//variáveis locais
   VTGrafico *grafico;
...
__fastcall TFormChild::~TFormChild(void)
   ...
   grafico->Close();
   //retira MultiPanel
   if (FormMultiMonitor) {FormMultiMonitor->RetiraMultiPanel(MultiPanel);}
   //destrói objetos
...
void __fastcall TFormChild::ActionBatchDensidadeCargaExecute(TObject *Sender)
   ...
   if ((form = DLL_NewFormBatchDC(this, apl)) == NULL) return;
...
void __fastcall TFormChild::ActionBatchImportaGisExecute(TObject *Sender)
   ...
   if ((form = DLL_NewFormBatchIG(this, apl)) == NULL) return;
   //exibe form como janela modal
   form->ShowModal();
   //destrói FormHistorico
   delete form;
...
void __fastcall TFormChild::ActionBatchPerdaregExecute(TObject *Sender)
   ...
   if ((form = DLL_NewFormBatchPR(this, apl)) == NULL) return;
   //exibe form como janela modal
   form->ShowModal();
...
void __fastcall TFormChild::ActionBatchPerdatecExecute(TObject *Sender)
   ...
   if ((form = DLL_NewFormBatchPT(this, apl)) == NULL) return;
   //exibe form como janela modal
   form->ShowModal();
...
void __fastcall TFormChild::ActionExportaTxtExecute(TObject *Sender)
   ...
   if ((form = DLL_NewFormExportaTXT(this, apl)) == NULL) return;
...
void __fastcall TFormChild::ActionExportaXmlExecute(TObject *Sender)
   ...
   if ((form = DLL_NewFormExportaXML(this, apl)) == NULL) return;
...
void __fastcall TFormChild::ActionImportaAnafasExecute(TObject *Sender)
   ...
   //cria TForm p/ importar a Rede
   if ((form = DLL_NewFormImportaAnafas(this, apl)) == NULL) return;
   //exibe form como janela modal
   if (form->ShowModal() == mrOk)
      {//exibe gráfico de topologia
      grafico->AreaRedeDefine();
...
void __fastcall TFormChild::ActionImportaAnaredeExecute(TObject *Sender)
   ...
   //cria TForm p/ importar a Rede
   if ((form = DLL_NewFormImportaAnarede(this, apl)) == NULL) return;
   //exibe form como janela modal
   if (form->ShowModal() == mrOk)
      {//exibe gráfico de topologia
      grafico->AreaRedeDefine();
...
void __fastcall TFormChild::ActionImportaXmlExecute(TObject *Sender)
   ...
   //cria TForm p/ importar a Rede
   if ((form = DLL_NewFormImportaXML(this, apl)) == NULL) return;
   //exibe form como janela modal
   if (form->ShowModal() == mrOk)
      {//exibe gráfico de topologia
      grafico->AreaRedeDefine();
...
void __fastcall TFormChild::ActionStkExecute(TObject *Sender)
   ...
   //exibe/esconde STK
	if   (ButStk->Down) sinap_child->FormStkShow();
	else                sinap_child->FormStkHide();
   }
...
void __fastcall TFormChild::AtualizaCaptionActionFlow(void)
   ...
   ActionFlow->Caption = "Power Flow " + IntToStr(mercado->AnoAtual);
   }
...
void __fastcall TFormChild::AtualizaCaptionForm(void)
   ...
   txt.sprintf("Network %d: Database ", Tag);
   ...
   txt = txt + " of " + DateToStr(redes->DataReferencia);
   //atualiza Caption
   Caption = txt;
   }
...
void __fastcall TFormChild::AtualizaJanelas(void)
   ...
   if ((butFlow->Down)||(ButAberturaMonopolar->Down)) grafico->Show(grafFLOW, ano, ind_pat);
...
bool __fastcall TFormChild::ExecutaCalculoFluxo(void)
   {
	...
	try{//verifica se deve executar fluxo por rede
...
void __fastcall TFormChild::FormActivate(TObject *Sender)
   ...
   if (FormMultiMonitor) FormMultiMonitor->ExibePanel2(MultiPanel);
   }
...
//---------------------------------------------------------------------------
void __fastcall TFormChild::InsereMsgAlerta(AnsiString txt)
	{
   //variáveis locais
   VTAlerta *alerta = (VTAlerta*)apl->GetObject(__classid(VTAlerta));
...
void __fastcall TFormChild::PopupModoGraficoPopup(TObject *Sender)
   ...
   if      (redegraf->ModoGrafico == modoGEOREFERENCIADO) MenuItemGraficoGeo->Checked = true;
   else if (redegraf->ModoGrafico == modoESQUEMATICO)     MenuItemGraficoEsq->Checked = true;
   else                                                   MenuItemGraficoFun->Checked = true;
   }
...
int __fastcall TFormChild::SalvaEstudo(bool confirma)
   ...
   if (estudo->Novo || estudo->Alterado)
      {//verifica limitações da Licenca de uso
      if (! ValidaLimiteLicenca()) return(IDNO);
      //verifica se deve pedir confirmação do usuário
      if (confirma) resp = Confirma("Deseja salvar o planejamento na base de dados ?", "", MB_YESNOCANCEL);
      else          resp = IDYES;
      }
   switch(resp)
      {
...
//---------------------------------------------------------------------------
bool __fastcall TFormChild::SalvaRedeComo(void)
   {
   //variáveis locais
   bool           sucesso;
   bool           base_nova = true;
...
TWinControl* __fastcall TFormChild::SelecionaParentParaJanela(void)
   ...
   if (MultiPanel) return (MultiPanel->PanelAtivo);
...
void __fastcall TFormChild::SelecionaPatamarMaiorDemanda(void)
   ...
   VTRedes    *redes    = (VTRedes*)apl->GetObject(__classid(VTRedes));
	...
	try{//determina Patamar com maior fluxo de potência
...
void __fastcall TFormChild::SinapToolKitInicia(int stk_option)
   ...
   if (licencas->LicencaTipoDemo()) return;
   //inicia SinapToolKit
	DLL_FormChildStart(this, apl, stk_option);
   }

//---------------------------------------------------------------------------
bool  __fastcall TFormChild::ValidaEqptoIsolado(void)
   {
   //variáveis locais
   int     resp = IDYES;
   VTRedes *redes = (VTRedes*)apl->GetObject(__classid(VTRedes));
...
void __fastcall TFormChild::VerificaModulos(void)
   ...
   //insere Comandos em FormStk
	sinap_child->FormStkAdd("SINAP: Basic Components", ActionListBase);
